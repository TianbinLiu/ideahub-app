...
src/
  ...
  data/        本地库（IndexedDB）与账号库，含种子数据与迁移；
               `boot.ts` = 开机装载的**唯一**一道闸（哪几样读不出来整页拦、哪几样在用到的那一屏降级，调研与出处见 docs/local-storage-failure.md）；
...
- **流水线只有一份**（2026-08-30 主人点名"两个模式的节点数据是同一份"）：节点数据的
  ...
  **三种卡的分工（V3，2026-09-06 主人拍板，`types.CARD_SLOTS` 头上那段 ★★）**：场景卡 = 地点（卡面当定场参考图）；
  ...
  **每种卡的文字只写自己负责的那一部分**（2026-09-18 主人点名，口径只在 `ai/cardScope`）：所有写卡片文字的提示词（自传图做卡片的 AI 车道、
  拍照识别、铸卡师、组稿提卡、模板提卡）都拼这份分工表 —— 人物卡只写这个人本身（长相、发型、体型、服装与佩戴的饰物），不写动作、
  手里的东西、地点背景、画风；场景卡不写人；道具卡不写谁拿着、在哪；风格卡不写具体内容。**简介（summary）不进任何出片 / 推演 /
  画卡面 / 提卡组提示词**（背景卡除外：它的简介就是故事），没写出片句的卡只报卡名（`types.idLineOf`），出片句也不再带名字—— 原来兜底「名字：简介前 40 字」，
...
- **方案有结构化镜头字段**（`types.ShotSpec`：景别 / 运镜 / 情绪节拍，2026-09-06 对标 updream 分镜 Skill）：推演按字段写、
  ...
  **结构化技能**（`studio/structuredSkills`，2026-09-06 对标 updream Skill）：与「出片技能」（一句话，`data/agentSkills`）不同，
  ...
  形状检查失败不退也不再扣。演示构建走本地按句号切段（面板标「演示」），不冒充模型。模型那一发走 `skillChat`
  （`arkClient.chatBounded`：上限 `SCRIPT_SHOTS_MAX_TOKENS` = 2400、超时 170 秒，都是实测定的），**不走 `chat()`** —— 它的 800 上限
  实测会把一半的 8 段输出截断；顶到上限如实报「被截断、这一次已经计费」，不去解析半截 JSON。
...
- **成片合并走系统硬件编解码器，不在 WebView 里录屏**（2026-09-07 主人点名）：原生 `VideoMergePlugin`
  （AndroidX Media3 Transformer）做拼接 / 逐段裁剪 / 画幅归一 / BGM 循环混音 / 标识叠加，输出**标准 mp4**；
  Web 侧只有 `utils/nativeMerge` 这一个入口。**段落传公网地址**让它流式取 —— 段在出片那一刻就转存到图床了，
  先下载再喂进去等于把几十兆搬两趟。只有**本地挑的 BGM** 要经 `stageFile` 落盘（它在 Web 侧是 `blob:`，
...
- **AI 生成标识分两顶帽子，都要戴**（2026-09-07 逐条查证，出处见 `data/aigcLabel` 头部）：
  ...
  ⚠ 别写成"第四条全条没有『持续』字样"（我写错过一次）：第（五）项虚拟场景那条就有「**可以**在虚拟场景**持续**
  服务过程中……添加」。这反而**加强**结论 —— 立法者知道怎么写「持续」，虚拟场景那项写了且是「可以」，
  对视频连"可以持续"都没写。另：「起始画面**和**播放周边」是并列不是二选一，而播放周边是我们自己的 UI、
  **不跟着文件走**，所以起始画面那份必须烧进像素。
  所以画面内那份只在**起始画面盖 ≥2 秒**（国标 GB 45438-2025 的量化下限：起始画面的边或角、
  字高 **≥ 画面最短边 5%**、**≥2 秒**、文案含「AI」+「生成」）。逐帧角标是 2026-09-07 之前自己加码的。
  ★ 字高按**最短边**算，不是按高 —— 按高算横屏那头会不合规。
  ② **传播侧**（第六条）：发布内容**周边**要有提示 —— `components/AigcBadge`，首页 / 详情页 / 发现页三处。
  B 站、抖音只戴这一顶（它们是传播平台），我们**两顶都戴**，照抄它们会漏掉生成侧那一半。
  ⚠ 第四条第二款：**导出的文件里**必须有显式标识 ⇒ 不能全改成播放器上叠 DOM。
  ⚠⚠ **还缺一格**：隐式标识（第五条，"应当"）—— GB 45438 的 AIGC 元数据字段写进文件，我们今天大概率是零。
  数字水印只是"鼓励"顶不了它。执法通报点名的全是**缺项**，不是"显示得不够久"。
  ⚠ 图片那条路：方舟 `watermark` 默认 **true**，而 arkClient 传 false —— 图片是真把标识摘掉了
  （视频那条相反，默认就是 false）。设定帧 / 卡面 / 封面若单独对外展示要自己补。
...
| 两仓价目表各写各的 | 页面报价 ¥25、实际扣 ¥15，用户觉得被偷钱 | `src/data/economy.ts` 是**报价**，server 的 `payment/order.service.js` + `config/tokens.js` 是**结算**，必须逐条相等。server 的 `payOrder.spec.js` 末尾钉了一份（只钉充值包与套餐；出图单价与 chat 定额钉在 `arkProxy.spec.js`）。⚠ 服务端按**调用**计价、不按内容量：看图（`chatVision`）塞几帧都是一次 chat = `CHAT_TURN_TOKENS`，按帧报价在 2026-08-17（白模）与 2026-09-10（四条提卡路 + `mintCards` 记账）各栽过一次 |
...
| 单轮 chat 的 800 输出上限拿去吐整份结构化 JSON | 「剧本 → 分镜」要 8 段带镜头字段的 JSON，2026-09-11 直连方舟实测 6 次输出 741~925 token、**一半超过 800**：`chat()` 把它拦腰截断 ⇒ 解析失败、面板只说"读不出来"，而服务端已经按 chat 收过钱，全程没有一个字提"被截断" | 输出长度跟着形状涨的调用别用 `chat()`：走 `arkClient.chatBounded`（上限与超时由调用点给，回报 `finish_reason:"length"`）。上限**先量再定**、量法写在常量旁（样板 `structuredSkills.SCRIPT_SHOTS_MAX_TOKENS`）；截断要当面说成人话并说清钱花没花；超时要比服务端 `/api/ark` 的 150 秒长（客户端先放弃 = 服务端照扣、用户收到失败） |
...
| 工坊投影窗里选定模板 / 挑定走向之后，第一段的 ‹ 灰着、删段被"只剩一段"挡住 | 「回铸段窗重选模式」只给空白段，白模段选完模板就没有退路，用户描述成"选择走向后返回键是灰色不可用"（2026-09-06 主人真机） | 判据放宽成 `flowStore.nodeRecastable`（还没出片、没在炼）：第一段的 ‹ 就是「回铸段窗」（`studioStore.recastNode`），推演过三套的段先弹确认卡（方案花过 token）；`removeNode` 的"只剩一段"闸也只挡已出片的。丢什么、花没花钱由投影窗那行小字如实说 |
| 工坊里一张**空白占位段**（加了段还没推演）把人困住 | 点节点卡开的是方案台：顶栏那枚 ‹ 是"上一段"（第 1 段时灰着），铸段窗那枚"上一步"随窗一起没了，「删除本段」又被"只剩一段"挡住 —— 用户描述成"退出工坊再进来，返回按钮消失了，没法重选走向"（2026-09-05 主人真机） | 空白的判据只有 `flowStore.nodeBlank` 一处（没剧情、没帧、没出片、非自定义/白模）：方案台给「‹ 回铸段窗重选模式」（`studioStore.recastNode`，撤段 + 按原要求/档位/画幅/素材重开铸段窗，不花钱），`removeNode` 对空白段放开"只剩一段"那道闸。**2026-09-06 放宽**：判据换成上一行的 `nodeRecastable`（还没出片的段都行），空白段只是它的子集。⚠ 别按"只有一套方案"判空白：做同款/老草稿的段也只有一套，里面是真内容 |
...
| 白模段的成片在回看里是哑的、声音键灰着 | 用「有声音的白模模板」出的片放出来没声（2026-09-06 主人真机）。这不是取流的问题：`BLOCKOUT_TASK` 钉着 `generate_audio:false`（edit 会连参考视频的歌一起复刻，带歌模板会被方舟在输出端整发拒、钱不退），成片文件本来就没有音轨；而合并页的原片音轨预置只认分段组的 `group.sourceUrl`，普通白模模板一样哑 | 回看（`SegPlayer`）把模板那一段的原片音轨（`tpl.refVideo.url`，正是与这一段对齐的那段裁剪）用 `<audio>` 叠上去跟着视频走，给「🔊 模板原声」开关并说明一句；合并页预置也退到 `refVideo.url`（`studioStore.draftAudioHint`）。别去改 `generate_audio`：那是版权拦截换来的 |
...
| 给 media3 的 `TextOverlay` 返回**空文本**（想让角标「过一会就不显示」）| **整条合并当场炸**，而屏幕上那句话与角标毫无关系：`ExportException` 最外层只有一句分类词「Video frame processing error」。更坏的是它**不在第 0 帧发作** —— 前 2.5 秒文本非空、一切正常，硬件转码 20.7 秒的片子只要约 0.6 秒就越过那条线，于是表现成「刚开始合并就失败」，用户与开发者都不会往角标上想（2026-09-07 主人真机，反编译 media3 字节码才定的案）| 机理：`TextOverlay.getBitmap()` 拿 `getText()` 去 `measureText` 量宽 → 空串得 0 → `Bitmap.createBitmap(0, h, ARGB_8888)` 抛 `IllegalArgumentException`。**要「什么都不画」就返回一个空格**（宽度非零、画出来全透明），别返回空串。⚠ 同一次踩到的第二格：`brief(e)` 只取最外层 `getMessage()`，把唯一有用的那句 cause **扔了** —— 这类「分类词在外、真因在内」的异常，压成一句话时必须**走完整条 cause 链**，并原样 `Log.e` 一份进 logcat |
| 剪辑页合并前的「老草稿自救」用**阻塞式** `POST /transfer-video` | 一发请求里等服务端跨境搬 20~80MB，而中间挡着 Cloudflare 的 125 秒读超时 ⇒ 等满约 145 秒整发作废、**静默**退回方舟直链，于是原生合成器只好跨境流式拉 TOS（2026-09-07 真机：客户端 20:23:23 已经放弃，服务端后台 20:24:32 才搬完，那条 `[ark-transfer] done` 白打了）| 改走**受理式 + 短轮询**（`arkClient.requestArkTransfer`（`wait:false`）+ `transferStatus`）：每一发都只有几百毫秒，碰不到那堵墙，还能被「取消合并」打断。⚠ 这一步真的会花一两分钟，**进度文案必须一直在动**（现在带「已等 N 秒」）—— 一动不动的一句话既分不出「还在搬」与「卡死了」，也分不出卡在提交那一发还是等搬完 |
| 一条**只在内存里**的关键状态，被漏在了"重启"的另一边（`studioStore.draftAudioHint`）| 白模复刻段的成片文件**天生无声**（`generate_audio:false`，版权拦截换来的），声音全靠剪辑页那条「原视频音轨」预置在合并时混进去。而这条预置只在**组稿那一拍**（`finalizeInner`）算一次、只活在内存 store 上，`persistCutDraft` 存的只有 `draft`。于是只要 App 重启过（出包装机就会），再从个人页「接着剪」回剪辑页，音频页签是**空的** —— 合出来的成片整条没有声音，而屏幕上一个字都不说（音轨本来就是可选项，空着不构成任何错误）。主人 2026-09-07 真机报的是「转存后原本有声音的又没声音了」——**与转存无关**：同一个上传函数搬过的另外两条产物 `-wadfinal` / `-recover-wm14` 至今带着 `aac,audio` 好好躺在 Cloudinary 上（ffprobe 逐条量过 15 个文件），转存是逐字节的 | 预置跟着稿子一起落盘（`cutSession.CutSession.audioHint`，`saveCutSession(draft, audioHint)` 把它钉成**必填**参数——漏传是零症状的），「接着剪」那一跳一并还原（`ProfilePage` 的 `useStudio.setState`）。⚠ 更要紧的是**把哑片说出来**：合成器自己知道（`ExportResult.audioMimeType`），插件回 `hasAudio`，剪辑页随导航把话带去发布页（写 `err` 活不过换路由，见上面那格）。判**否定**：老插件不报这一位时当"不知道"，别乱说。⚠ 同族反面样板：谁要开了 `experimentalSetForceAudioTrack`，`hasAudio` 就会对一条**静音轨**报 true |
...
| 合成这种**能活过页面卸载**的长活没领票（`data/jobs`），成功那一拍直接 `navigate` | 屏幕上写着「可以切走」，人真切走之后：① 没有任何地方显示它还在跑；② 跑完那一拍把正在别的页面上的人**无预警拽到发布页** —— HashRouter 的 `navigate` 在组件卸载后照样生效（`useNavigateUnstable` 的 `activeRef` 只在 layout effect 里置真、**没有 cleanup**，那道闸拦不住）。而这期间从个人页「接着剪」回来再点「下一步」，原生按 BUSY 拒，上层拼出「**合并失败**：已经有一炉在合并了」—— 正文是真的，假的是"合并失败"四个字：那一炉好好地在跑，而这句话把用户推向最不该走的那条路（再合一次）| 领票：`startJob({kind:"merge", …})`，屏幕与票走同一个 `say()`（两处各写各的必然分叉）。成功时**判 `aliveRef`**：人在就 `done({silent:true})` + 跳页，人不在就 `done({msg, route:"/publish"})` 留一张可点的票。⚠ `warns` 必须跟着票走 —— 里面装着「这条成片没有声音」这类只有这一次能说的话，不带上就是"悄悄写完什么都不说"。⚠ 早退分支（取消 / BUSY）要在 `finally` 里把票撤掉，否则胶囊里挂一张永远转圈的。⚠ BUSY 那句话让**原生**自带出路，上层见 `code === "BUSY"` 不加"合并失败"前缀 |
| 原生插件里 `Transformer.cancel()` 与整段组装都裸跑，没有 try | Capacitor 的 `Bridge` 对插件方法抛出的异常是 `throw new RuntimeException(ex)` 到它自己的线程 ⇒ **进程当场死 + JS 那边的 Promise 永不 settle**（屏幕停在「合成中…」，连错误都没有）。`cancel()` 更狠：它跑在主线程 Runnable 里，而 `Transformer.cancel()` 会把释放阶段的 RuntimeException 重抛出来 | 组装那一段包 try/catch → `call.reject`（要用到 try 之后的局部变量就先声明成 final、在 try 里赋值，catch 里 `return`，Java 能证明定值）。`cancel()` 包 try/catch 之外**还要把 `call.resolve()` 放 finally** —— 取消本身失败了也要让上层往下走，否则 `cancelNativeMerge()` 的 await 永远悬着 |
| 给 BGM 挂 `ChannelMixingAudioProcessor` 只为了调音量 | 它是拿来**混声道**的，调音量只是副作用 —— 而 `ChannelMixingMatrix.createForConstantGain` **只实现了少数几种组合**（字节码里的报错原文：「…->1 are not implemented.」「…->2 are not implemented.」），能安全登记的只有 1→1 与 2→2 ⇒ 一条 5.1／7.1 的配乐会因为「找不到对应的矩阵」被**整发拒**，屏幕上一句英文，而用户能做的只有把配乐删掉重来 —— 提示里还不会这么说 | **自己写一个与声道数无关的增益处理器**（`VideoMergePlugin.GainAudioProcessor`，继承 `BaseAudioProcessor`）：「把音量乘 0.6」这件事根本不关心有几个声道，逐个 16-bit 样本乘一下就是了。⚠ 只接 16-bit PCM，别的编码 `onConfigure` 回 `AudioFormat.NOT_SET` ⇒ `isActive()` 为假 ⇒ media3 **跳过**这一环（不是报错：配乐照样出声，只是音量旋钮对它不起作用）。⚠ 字节序跟着缓冲区自己走（media3 给的是 nativeOrder），手写 `& 0xFF` 拼错了听感上是一片噪音、而不会有任何报错 |
| 几十兆的本地音频过 Capacitor 桥编成**一整条 base64** | 那是 1.33 倍体积的**单个 JS 字符串**，而同一份数据同时躺三处（JS 侧拼串、桥上复制、Java 侧整块 decode）—— 慢，且在低端机上有实打实的 OOM 面 | 按 512KB 切片逐块送（`nativeMerge.STAGE_CHUNK`），第一拍带 `ext` 建文件、之后带**上一拍回来的** `path` 续写。⚠ 原生那侧只认自己 `staged` 目录下的路径 —— 直接拿调用方给的路径写文件等于开了一个任意写的口子 |
| 剪辑页「音频」只有「挑本地文件」一个入口 | 预置的「原视频音轨」一旦被 ✕ 掉、或一开始就没预置上（取回段 / 老剪辑稿），用户**没有任何办法**把模板原声拿回来，而屏幕上也没有一个字提过它曾经存在 —— 一个只能失去、不能取回的东西 | 空态摆一颗「🔊 用模板原声」，数据取 `studioStore.draftAudioHint`（组稿那一拍算的那份，已随剪辑稿落盘） |
...
| 按错误 message 里的**中文关键词**判「是哪一种失败」（`/网络失败/`、`/网络不可用/`、`/审核/`） | 两头都零报错：措辞一改、或多语言之后 message 变成英文，分流就悄悄全落兜底句；更要命的是被分错的那一档往往关系到钱 —— 「没收到回包」恰恰是**可能已经扣了**的那一档（服务端先扣钱再转发，而客户端 chat 超时 120 秒短于服务端转发上限 150 秒），在这里说「没扣钱」是往放心的方向说错 | 认**类型 / 状态码 / code**，不认 message：`arkClient` 的 `ArkNoReply`（没收到回包，只能说「可能已扣」）/ `ArkBadReply`（2xx 读不出来，已扣）/ `ArkHttpError.status`（服务端网关连不上方舟回的是 **504** + `ark upstream TimeoutError`）；`api/client` 的 `ApiError.code`（NETWORK / TIMEOUT），上传分块断线的 `chunkError` 也带同一个码。浏览器自己的英文（`Failed to fetch`）不是我们的文案，留作兜底可以。2026-09-10 多语言第 1 步把 `briefArkReason` / `chatFailLine` / `videos.errText` 三处收了（`videos.errText` 2026-09-11 改名 `uploadFailOf`、改回码：待发队列落盘存码不存话，见 `PendingErrorCode`） |
| catch 里不分错误类型一律说「没扣钱」 | 远端模式（正式包）下**往放心的方向说错**，零报错：`spendTokens` 在远端是空操作，钱由服务端在 /api/ark 上**先扣、再转发**，只有上游非 2xx 才退（契约「扣费」一节，实现在 server `arkGateway.chargedArkCall`）。于是四种失败屏幕上都是「没画成」，钱却已经或可能已经扣了：① 没等到回包（`ArkNoReply`：断网、客户端先超时，服务端那一发照样跑完计费）② 2xx 但结果用不上（`ArkBadReply`）③ 图已出、取回失败（`real.genImageAsDataUrl` 的 `toDataUrl` 排在已结算的 `generateImage` 之后）④ `real.portraitViews` 逐格出图画到第 k 格才失败 —— 每格是一次独立的 /images/generations，前 k-1 张各自结算过了。2026-09-17 之前自传图做卡片的圈选改图 / AI 生成图位 / 人物信息、提取窗的炼形象图、导演台融图五处都这么说；唯一分了档的 `CustomCardPage.recognize` 又把人指去「我的」页的「钱包流水」——App 里**没有**那一页（`api/wallet.fetchWalletLedger` 零调用方，「我的」页与钱包抽屉只有余额） | catch 里要提钱只走 `ai/failCharge`：`chargeOnFail(e)` 是**唯一判定**（认错误**类型**、认 `isRemoteMode()` 与管理员免扣费 `billingExempt()`；判成「可能已扣」时顺手刷一次钱包，那一档恰恰没有回包、响应头带不回余额），`chargeNote(结论, 单价)` 给钱上的那句话（全仓只有这几句，当占位符嵌进各处自己的整句；回 null = 真的没扣，说原来那句，离线 / 演示构建因此一个字不变）。★ 规则的另一半在**抛错那一侧**：凡是「2xx 之后才坏」的失败都得抛 `ArkBadReply`（回包里没有图、图取不回来、模型回的不成 JSON），逐个计费的批量调用抛 `ArkBatchPartial` 带上已经结算的次数 —— 抛裸 Error 就静默落进「没扣钱」。★★ 批量里已经画好的那几张**不许跟着抛错一起丢**（2026-09-17）：逐格出图抛的是子类 `real.PortraitViewsPartial`，图挂在它的 `drawn` 上，调用方（`CustomCardPage.runAiForge` / `VideoCardAnnotator.makePortraits`）**收下 + 离线账本按张记上**两件一起做，下一次只补剩下的格子（报价 / 余额门 / 真画 / 离线实扣都读各自页里的那一份 plan）；`chargeOnFail` 离线时也照 `settledBefore` 说「已计费」—— 只做一半的话，离线要么白拿图、要么那句「已计费」是假话。⚠ 别对 `ArkBatchPartial` 直接 `instanceof ArkNoReply`（恒假），拆壳只在 `chargeOnFail` / `briefArkReason` 里。⚠ 指给用户「去哪儿核对」之前先确认那一页真的存在（现在指的是「我的」页的 token 余额）。⚠ 新接进来的调用点先核对「成功之后才 `spendTokens`」：先扣后调的话，离线模式那句「没扣钱」就不成立了 |

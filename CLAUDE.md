...
| 上传大视频失败，只说「网络不可用」 | 2026-08-21 晚真机上传 47MB 素材失败（fetch 直接 reject、**没有状态码**），同一台设备几分钟后 11.6MB 成功。**n=1，连"可复现"都还没证实** | ⚠ **这一格底下曾经写着一个我自己编的原因（"Cloudflare ~100 秒读超时"），它是错的**，08-22 查证后删掉 —— 记在这里是因为它两跳之内就从一句注释变成了组织记忆。**已排除的**（别再往下传）：① 体积超限 —— 四道尺子全宽于 47MB（客户端 100MB / multer 100MB / nginx `client_max_body_size 110m` / CF Free 100MB），且 08-22 从**那台手机自己** curl 发 47MB 到同一端点，68 秒完整传上去、无 413；② "~100 秒时间墙" —— CF 官方超时表里根本没有这条（真实值：Proxy Read 125s→524、Proxy Write 30s→524、客户端侧 400s keep-alive），且实测一发 147 秒的慢速请求完整走通；③ 客户端自超时 —— 600s，走的是非 abort 分支；④ pm2 内存墙重启 —— 故障时刻两个 worker 已连跑 26 小时未重启。**还没排除的**：nginx `client_body_timeout`（没设 → 默认 60s，语义是"两次读之间"，移动网络一次 ≥60s 停顿就 408 断连，而 408 只记 info 级、不进 error.log）、Cloudinary 那一腿（`upload_stream` 没设 timeout，且是**同步等它传完才回包**）、手机侧链路事件。**下一步只有一条**：读 nginx `access.log`（deploy 不在 adm 组、无 sudo，读不到）—— 有那条 POST 就说明到过源站，没有就说明卡在 CF 或上行 |

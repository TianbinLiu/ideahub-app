...
src/
  ...
  data/        本地库（IndexedDB）与账号库，含种子数据与迁移；
               `boot.ts` = 开机装载的**唯一**一道闸（哪几样是核心库、没打开时整页怎么说、哪两样可降级）；
...
| 开机装载（`data/boot` 里那一排 ready*）有一样没打开 | 原来是 `void Promise.all(...).then(setReady)`：任何一个 reject，页面就**永远**停在「正在打开作品库…」，不说原因、没有出路。⚠ 更常见的反而**不 reject**：`db.idbGet` 把一切读失败答成 undefined，`open()` 失败一次整个会话都是 undefined —— IndexedDB 打不开时 App 照常开机，草稿箱空、离线账号被登出、作品只剩种子，而下一次写入（存草稿 / 点赞 / 铺种子）会拿这份空表把磁盘上的真数据**整张盖掉** | 开机闸只有 `data/boot` 一处。装「存着用户东西的库」一律用 `db.idbRead`（读失败抛 `IdbError`，undefined 只表示没有这个键）；核心库失败 = 整页 `EmptyState full` + 「重试」（重跑装载，不 reload）。可降级的两样（互动记录 / 弹幕）自己 catch、**冻结落盘**、在用到的那一屏说一句。⚠ 装载失败要把半截状态撤掉（account 的 `db`、videos 的 `cache` 与 `cacheOwner`），否则 `if (db) return` 让「重试」直接拿到一个半截库。⚠ 失败页**不给「清理缓存」**：它碰不到让开机失败的任何东西，而它判"没人引用"读的正是那几个没装上的库，这时去扫会把草稿还指着的合并成片当孤儿删掉 |

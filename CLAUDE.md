...
| 上传大视频失败，只说「网络不可用」 | 2026-08-21 真机实测：**47MB 传不上去、11.6MB 同一分钟传得上去**，而提取器那一屏白纸黑字写着「100MB 以内」。fetch 只会给一个 TypeError —— 网关对请求体大小有上限时（nginx 那类先回 413 再断连），浏览器侧就退化成网络错误，`res.ok` 那条路**根本走不到**，所以状态码判不出来 | 客户端能做的只有把**这份多大**写进话里（`api/uploads.ts` 的 postForm，已加）并给一条出路；真正的上限在**服务端/网关**，`TEMPLATE_UPLOAD_RULES` 与那句「100MB 以内」在对齐之前都只是我们的一厢情愿 —— 改之前先量出服务器实际收得下多大 |

...
| 上传大视频失败，只说「网络不可用」 | 47MB 失败、11.6MB 成功；客户端只拿到 fetch reject（**没有状态码**） | **2026-08-22 用 nginx access.log 定案到这一步**：请求**到过源站**，记的是 **499**（前面那一侧在源站回包之前关了连接），不是 413、不是 524 ⇒ **与体积无关、与 CF 套餐无关**（Free 上限本就是 100MB，Pro 也是 100MB）。两发对照：47MB `14:10:03→14:12:09` **约 126 秒 / 499**；11.6MB `14:15:25→14:17:01` **约 96 秒 / 200**。⚠ nginx 的 `$time_local` 是**请求结束**时刻（Apache 才是开始）—— 这条日志自己就证明了：客户端拿到预检 204 会立刻发 POST，两者之间那 126/96 秒只能是 POST 自身耗时。手机上行实测 0.69MB/s ⇒ 11.6MB 里真正传字节只占 17 秒，**剩下 ~79 秒全在服务端**：`multer memoryStorage` 把整份读进 Node 内存 + `cloudinary.upload_stream` **同步等 Cloudinary 吃完才回包**。47MB 的那一段按体积放大就是四五分钟，于是整发在 ~126 秒被掐。⚠ **不是一道固定的时间墙**（别再往这个方向猜）：同日实测一发**持续 147 秒**、全程有数据在流的慢速请求完整走通，而 11.6MB 那发传完后静默 ~79 秒也活了下来 —— 没有单一阈值能同时解释这三行。断在"还在传"还是"传完等回包"，默认日志格式分不出来（要加 `$request_time $request_length $upstream_response_time`；⚠ 别写 `$body_bytes_received`，nginx 没有这个变量，`nginx -t` 会直接失败）。⇒ 能确定的只有一条：**病根是「让客户端一直吊着等 Cloudinary」** —— 文件越大、连接要存活越久，暴露面越大。又：客户端拿到的是 fetch reject（无状态码），而 CF 的 524 是**带状态码的真实响应** ⇒ 掐断发生在**手机↔CF** 这一腿。根治方向：**客户端签名直传 Cloudinary + 分块**（顺带拿掉 Node 堆里那份 47MB 拷贝与 pm2 400M 的风险）；在那之前别改上限、别升套餐，都不解决。⚠ 曾经这一格写过两个我自己编的原因（"体积超限"、"CF ~100 秒读超时"），都已证伪 —— 编出来的因果两跳之内就会变成组织记忆 |

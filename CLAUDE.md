...
| 给 BGM 挂 `ChannelMixingAudioProcessor` 只为了调音量 | `ChannelMixingMatrix.createForConstantGain` **只实现了少数几种声道组合**（字节码里的报错原文：「…->1 are not implemented.」「…->2 are not implemented.」），能安全登记的只有 1→1 与 2→2 ⇒ 一条 5.1／7.1 的配乐会因为"找不到对应的矩阵"被**整发拒**，屏幕上一句英文，而用户能做的只有把配乐删掉重来 —— 提示里还不会这么说 | 音量就是 1 时**一个处理器都不挂**（预置的模板原声正是 1.0），多声道源直接透传。⚠ 调过音量的多声道源仍会撞上 —— media3 这一版就是没实现，要绕只能自己写一个与声道数无关的增益处理器 |

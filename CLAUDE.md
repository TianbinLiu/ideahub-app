...
- **主按钮两档形状**（2026-09-05 主人定）：整宽 / 独立的 CTA 一律 `rounded-xl … py-2.5`（`w-full` / `flex-1` /
  空态里的「去创作」这类），顶栏、行内、列表行里的小键一律胶囊 `rounded-full`（高度按所在行）。此前 `bg-brand`
  按钮有十来种圆角 × 高度组合。查法：`rg 'bg-brand' --glob '*.tsx' | rg 'rounded-(lg|md)'` 应为空。

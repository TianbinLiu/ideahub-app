...
| 上传大视频失败，只说「网络不可用」 | 2026-08-21 真机实测：**47MB 跑到 ~100 秒被 reject，11.6MB 同一条 WiFi 传得上去**，而提取器那一屏白纸黑字写着「100MB 以内」 | **别按"大小超限"去查**：服务端 `MAX_TEMPLATE_VIDEO_BYTES` 就是 100MB、nginx `client_max_body_size 110m`，两边对齐着。按那次上行速度（11.6MB 一分多钟 ≈ 0.15MB/s）算，47MB 要四五分钟 —— 更像**中间代理的读超时**（Cloudflare 默认 ~100 秒）先掐了连接。⇒ 这条路真正的天花板是「这条网 ~100 秒内能推上去多少」，手机上大约十几 MB，与那个 100MB 差一个数量级。fetch 两种情况都只给 TypeError（`res.ok` 走不到、没有状态码），客户端判不出来，只能把**这份多大**写进话里（`api/uploads.ts` 的 postForm，已加）。要真正修：抬高代理读超时，或把「100MB 以内」改成实测得出的说法 |

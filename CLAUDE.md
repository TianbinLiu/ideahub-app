...
| 成片预览截帧走 fetch→blob，失败只 `console.warn` | 20 秒 1080p 的片子几十 MB，手机网上 120s 内拉不完 ⇒ 无 poster、无真实尾帧、无实测时长；画布上「成片预览没截到」，而 release 包看不到控制台，谁都不知道为什么 | 有 CORS 的地址（转存后的 Cloudinary）直连 `<video crossOrigin>` 靠 Range 只拉两帧附近的数据（`grabHeadTail`），不成再下载后截；方舟直链仍走代理。三处捕获失败都把原因写进步骤日志（`captureIssueLine`）。真机 WebView 上直连与 blob 两条路都验过能截 |

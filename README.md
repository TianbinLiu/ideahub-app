...
- **节点链**：点击虚线空白卡位 → 卡组在下方展开（箭头翻页，可拖拽卡片到卡位）+ 节点编辑窗（预览图 / 按类型的素材槽 / 视频要求 / 时长 AI 定或自定义）→ 生成**三个方案**（上中下三张节点卡，各含首尾帧 + 小说式剧情）。点开方案窗（左首尾帧/右剧情）选定后其余收起，右侧光束连出新的空白卡位；下一段生成以**整条已选路径**为上下文（首帧承接上一段尾帧色调）。重选其它方案会收起原方案的后续子树（切回可恢复）。超出桌宽的最早节点自动收到左侧堆。
- **合成**：中线右端金色圆台 → 按时间线合成 → 编辑发布页（标题/分类/简介/从各段首尾帧选封面）。

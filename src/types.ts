 ...
 *
 * ★★ `"number"` 是**兜底档**：判据（`data/templates.isColorMark`）判的是"有没有
 *   `markColors`"，缺失一律落这一档。凡是新加的传递层漏带这一位，故障形状就是
 *   "颜色模板被当成编号模板"，写出来的那句话长成 `编号绿色=凛` —— **一眼就是坏的**，
 *   而且摆在用户花钱之前那个可编辑的输入框里。反过来（默认颜色）则会让老模板
 *   写出「把绿色人偶替换为…」而画面上根本没有绿色，一样错、却完全看不出来。
 *   所以这个默认方向不是随手定的，别翻过来。
 * ★ 判据不在这里 —— 类型只是给这两档起个名字，"一个模板是哪一档"的唯一实现在
 *   `data/templates.isColorMark`。
 */
export type MarkScheme = "number" | "color";
...
export interface VideoTemplate {
   ...
   * ★★ `label` **就是人偶身上那个标记本身**，两种形态并存，靠 `markColors` 分辨：
   *     · **颜色方案**（2026-08-16 起新做的模板）：label 是色名（"绿色"/"黑色"…），
   *       人偶通体一色。颜色是材质，转身、侧对、被挡住再露出来都还是那个颜色。
   *     · **编号方案**（存量老模板）：label 是阿拉伯数字（"1"/"2"/"4"…），实测**稳定但
   *       不连续**（2026-08-15 一发四个人偶实出 1/2/4/5），而且**只印在人偶的某一面**
   *       （多半是额头或后脑）—— "四面都印同一个数"那条老承诺从没被模型执行过。
   *   两种形态共用同三条禁令 ——
   *     · 别按下标推 label（`roles[i]` 的标记**不是** `i+1`）；
   *     · 别拿 `roles.length` 当"最大编号"；
   *     · 点名提示词里必须**原样用 label**，不许重编、不许换成近义词：对着 4 号说 3 号、
   *       把"绿色"写成"青色"，模型就换错人，而"画面上换了个人"这件事没有任何报错，
   *       只有作者肉眼能发现。
  ...
  /**
   * 阶段一白模化时**真正写进那段提示词**的那份颜色清单（逐字、按序）。
   *
   * ★★ **存在即颜色方案，缺失即编号方案** —— 这一位就是本次改动的判据本身，判定只写
   *   存在性（唯一实现是 `data/templates.isColorMark`）。线上那两个还在用的老模板
   *   （人偶身上印的是阿拉伯数字、身体是白的）天然缺这一位，于是天然落进编号方案、
   *   套用时走老提示词，一个字都不受影响。反过来写成 `!== "number"` 之类的肯定式，
   *   会把存量整批翻面 —— 画面上根本没有绿色人偶，提示词却说「把绿色人偶替换为…」，
   *   当场作废且零报错。这是本次改动的头号红线。
   * ★ 它不只回答"是不是颜色方案"，还给出三件今天真的要用的东西：核对面板颜色选择器的
   *   候选集（**必须**限定在这段视频里真实存在的那几种颜色，否则会造出一个永远挂不上
   *   也永远不报错的死位子）、作者误删一个位子之后能把它加回来、以及"这段里只有这 N 种
   *   颜色"这句话说得准（删过位之后从 `roles` 推是错的）。
   * ★ `swatch` 是服务端按色名现查调色板派生出来的色值（**不落库**），只给 App 画色块用。
   *   查不到就没有这个键 —— 那时画中性灰块 + 纯文字，**永远不猜一个颜色**：猜错了作者会
   *   照着我们编的那个颜色去核对画面，那正是这条链路最怕的"以为核对过了"。
   * ⚠ 本仓**不许**出现任何颜色名或色值常量。调色板的唯一实现在服务端
   *   （`blockoutize.service.MARK_PALETTE`）：App 侧颜色的**文字**一律来自 `roles[].label`，
   *   **色块**一律来自这里的 `swatch`。这样"两边相等"从靠约定变成结构上不可能不等。
   *   看到有人在本仓加一个 `Record<色名, hex>`，就是这条设计被推翻了 ——
   *   回来改这段注释再动手。
   */

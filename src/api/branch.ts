...
export interface BlockoutizePayload {
   ...
   *   就是把同一条式子抄成两份，服务端改了公式我们不会知道，分叉的表现是"页面说看 6 帧、
   *   服务端看了 3 帧"，两个方向都不报错。（这里原来写着"帧数就是钱"—— 不对，看帧是一次 chat 定额。）
   * ★ 为什么这一个可以收客户端报的数、而 durSec 那一组不能：帧数**上限**由服务端夹，
   *   而且帧数不进价钱（多标几帧也还是一次 chat）；
 ...
 * ★ 409 = 识别中的那把并发锁（上一次还没跑完；服务端那句话原样透出）—— 不是错误，是"等一下再来"。

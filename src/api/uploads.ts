...
async function postForm(
  ...
  try {
    ...
    //   同一分钟内 11.6MB 传得上去。
    //   ⚠ **不要在这里写原因** —— 08-21 我在这行写过"多半是请求体大小超限"，08-22 查证
    //     发现与服务端配置直接矛盾；改写成"多半是代理 ~100 秒读超时"，再查发现 CF 官方
    //     超时表里没有这条规则，且实测 147 秒的慢速请求完整走通。两次都是**编的**。
    //     已排除：体积（四道尺子全宽于 47MB，且从同一台手机 curl 发 47MB 68 秒传完）、
    //     ~100 秒时间墙、客户端自超时（600s，非 abort 分支）、pm2 内存墙（故障时未重启）。
    //     还没定案的候选与查证方法记在 CLAUDE.md 的坑表里，**别在这里复述**（复述一次，
    //     下一个人就会把它当结论）。

...
/**
 * 「这**整句**说的是段的属性，不是画面」—— 本地档办不了这些（时长/画幅/画质在 ⚙ 本段设置里，
 * 删段在卡片上），所以宁可说没听懂，也别把它当成拍摄要求写进去（见下面 ★★）。
 *
 * ★★ 必须**整句锚定**（2026-08-21 验证轮抓到：第一版是在句中找关键词，四个正常句子全被打掉
 *   ——「拍一只猫在**高清**屏幕前打滚」「主角说要**删掉**那封信」「**标准**的日式庭院」
 *   「镜头**横屏**移动」）。属性指令天生短、且整句就是那个词，描述画面的句子天生长且带动词，
 *   两者在"整句就等于这个词"这个形状上不重叠。判错的代价不对称：把属性句当要求 = 静默覆盖
 *   用户写好的一大段（不可撤销）；把要求句当属性 = 白拒一次（他一个字没丢，但会一头雾水）。
 */
const ATTR_ONLY =
  /^(?:改成|换成|变成|设成|调成|用)?\s*(?:\d+\s*(?:秒|s)|时长|竖屏|横屏|画幅|画质|极速|标准|高清|电影级|删掉|删除|去掉|短一点|长一点)\s*(?:吧|了|一?点)?[。.!！]?$/;
...
function localParse(text: string): { say: string; ops: Op[] } {
  ...
  for (const part of text.split(/[;；。\n]/)) {
    ...
    if (m) {
      ...
      else if (ATTR_ONLY.test(rest)) attrs.push(`第 ${seg} 段`);
  ...
  const attrSay = attrs.length
    ? `${[...new Set(attrs)].join("、")}说的是这一段的设置：时长/画幅/画质点卡片上的 ⚙ 改，删段用编辑窗底部的「🗑 删除本段」。`
    : "";
  return {
    say: ops.length
        ...
        applied.push(
          nodeDone(node)
            ? // ★ 指名道姓说在哪儿（旁边每句拒绝语都指了，就这句没指）：已出片的段编辑窗里
              ...
              `第 ${o.seg} 段：要求已写（这一段已经出片，新要求要等重新推演才作数——编辑窗点「📋 看/改这一套方案」，里面有「♻ 重新推演三套」）`
            : `第 ${o.seg} 段：要求已写`,
        );
        break;
      }

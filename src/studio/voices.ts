 ...
 * 默认语调指令：**二次元御姐**。
 *
 * 上一版写的是"没有情绪、平铺直叙、像在读一份清单"——那是把"低沉"错当成了
 * "无起伏"。结果确实低了，但也念得像播报，人物没了。御姐感的来源其实是
 * **从容**（不赶、气息稳）和**疏离**（不主动讨好），不是没有情绪；
 * 所以这一版明确写"情绪是内敛的——有起伏，但克制"，把"不要有情绪"删掉，
 * 只保留"不外放/不甜/不撒娇/不刻意亲切"这些**反向**约束。
 *
 * 低沉那一半交给 pitch（-6）和语速（-20）去做，不靠文字描述硬压。
 */
export const DEFAULT_INSTRUCT =
  "你是二次元作品里的御姐型角色：嗓音低沉偏磁性，语速从容不赶，气息稳而略带慵懒；" +

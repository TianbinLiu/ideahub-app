...
export function blockoutApplySkeleton(cast: BlockoutCastSlot[], userLine = ""): string {
  ...
  if (free.length > 0) {
    ...
    // ★★ 但"原样不变"这四个字曾经把**编号也一起保住了**（见下面 ★★ 那条实测）：
    //   没挂卡的人偶要保持的是"白模的样子"，不包括它头上那个号 ——
    //   那是我们自己加上去的记号，不是画面内容。所以这里必须把编号单独摘出来说。
    parts.push(`${free.map((s) => `编号${s.label}`).join("、")}保持白色人偶的样子，但同样去掉编号。`);
  }
  ...
  parts.push("把人偶头上和身上的编号（数字、号码牌）全部去掉，成片里不许出现任何编号或数字。");
  parts.push("不要出现字幕。");
  return parts.join("");
}
...
export async function composeBlockoutPrompt(cast: BlockoutCastSlot[], userLine = ""): Promise<string> {
  ...
  if (!/(去掉|去除|抹掉|擦掉|删掉|不保留|不许出现|不要出现|不得出现)[^。；]{0,12}(编号|数字|号码)/.test(text)) {
    throw new Error(

...
export interface BlockoutCastSlot {
   ...
   * ⚠⚠ 这一维**还没有被证明在 r2v 那头有效**：2026-08-18 花 ¥8.8 把话说进去了，
   *   换掉的仍然是最显眼的那个主角。但那一发被当时 18 字的上限把最扛事的空间锚点
   *   切没了（复盘见 APPLY_DESC_MAX），所以既不算证实也不算证伪 ——
   *   别把它当成一个已经验证过的机制去依赖，也别因为"反正没用"就把它删了。
   */
  mark: string;

...
export interface BlockoutCastSlot {
   ...
   *   ★★★ 而且第二发里那个红色的**本来就在"保持原样"的名单里**（`从左数第4个的人偶…
   *     保持人偶原样，不要替换成任何人`）—— 正向点名、负向点名、多维描述三样一起上，
   *     它照样被换。⇒ 这不是"哪个锚点认得出目标"的问题：r2v `edit` 在这类构图里
   *     像是**认准了画面里最显眼的那个主体**，文字改不动它。
   *   ⚠ 唯一还没试过的形态：**给"保持原样"那几位也带上描述**（现在它们是裸序数）——
   *     尤其是那个主角，让否定句里也有它的样子。这是下一发该测的东西。
   *   ⇒ 现状：这一维**留着**（它无害、且在没有主角的素材上仍可能帮忙 —— #46 里
   *     红色被裁到边缘、只剩 3 个人时序数本来就灵），但**别把它当成一个已验证的机制**，
   *     更别在它身上再叠新逻辑。要动之前先看这段注释。
   */
  mark: string;

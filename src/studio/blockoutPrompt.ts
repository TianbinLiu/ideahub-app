...
export function blockoutApplySkeleton(cast: BlockoutCastSlot[], userLine: string, spec: MarkSpec): string {
  ...
  //   ★ 这一句放在用户那句话**之后**：VIDEO_PROMPT_MAX 是从正文尾巴切的，
  //   而它属于"机器生成、不许被截掉"的那半 —— 排在这里，被截的先是用户自己的补充
  //   （那部分少几个字用户看得出来），不会让编号悄悄留在成片里。
...
export async function composeBlockoutPrompt(
  ...
  if (
    spec.scheme === "number" &&
    !/(去掉|去除|抹掉|擦掉|删掉|不保留|不许出现|不要出现|不得出现)[^。；]{0,12}(编号|数字|号码)/.test(text)
  ) {
    throw new Error(

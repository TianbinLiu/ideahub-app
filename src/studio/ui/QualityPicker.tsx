...
import { getQuality, setQuality, isNativeApp, QUALITY_LABELS, type Quality } from "../quality";
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          {(Object.keys(QUALITY_LABELS) as Quality[]).map((q) => {
            const disabled = q === "high" && isNativeApp();
            return (
              <button
                key={q}
                disabled={disabled}
                ...
                className={`flex items-center justify-between rounded-xl border px-4 py-3 text-left transition ${
                  disabled
                    ? "cursor-not-allowed border-slate-700/40 bg-black/10 text-slate-500"
                    : cur === q
              ...
              >
                <div>
                  ...
                  <div className="mt-0.5 text-[11px] text-slate-400">
                    {disabled ? "App 版未内置极致档大文件（控制安装包体积）" : QUALITY_LABELS[q].desc}
                  </div>
                </div>
                {cur === q && !disabled && <span className="text-brand">✓</span>}
              </button>
            );
          })}
        </div>

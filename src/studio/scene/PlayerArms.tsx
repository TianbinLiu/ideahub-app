...
> = {
  ...
  tsumire: {
    ...
    hideHead: true,
    // 眼位实测（站姿收敛后，均为"前表面距头骨"）：脸 0.32 / 衬衫领口 0.246 /
    // 背带裙 0.333 / 鞋 0.395——这个头身比下**裙子比脸还靠前**。
    // 取 0.26 = 脸表面后方 0.06，正好是眼球中心的解剖位置（脸在第一人称被藏掉，
    // 相机在脸皮内侧无妨），离领口顶 0.062、配 near=0.03 不切。
    // 沿用默认 0.12 相机会卡在颅骨里（body002 跨 z 4.61~5.15），低头必穿模；
    // 给到 0.34 又会越过裙子前表面，低头只看得到地板看不见自己。
    eyeOffset: [0.26, 0.04],
    ...
    springGroups: [
      ...
      // 猫耳**不进弹簧**：每只耳朵只有单独一根骨（无子链），弹簧解的是"骨→尾端方向"，
      // 单根骨遇上任何重力都会整只向下折——实测折平贴在头侧、正面完全看不见耳朵。
      // 试过用 Bone & Weight Splitter 把 Ear.L/R 各拆成三节来接物理：骨确实拆成了
      // Ear_1/2/3，但**权重没能随 glTF 导出保住**，耳网格顶点组仍指向已不存在的
      // Ear.L，两只耳朵直接从画面上消失。已回滚，维持当刚性头饰。
    ...
    // 碰撞球按**矢状截面**拟合，不是按"水平半径中位数"。
    //
    // 用户报"头发贴不到背上，像隔着一层空气墙"。根因是拿球去拟合躯干：躯干是**左右宽、
    // 前后薄**的椭圆，取周向中位数当半径，球就会从前后同时鼓出来，头发只能搭在球面上。
    // 从骨心朝各方向打射线量到身体/衣物表面（站姿收敛后）：
    //   Spine1 前0.173 后0.200 左0.153 右0.161  —— 而半径给了 0.27，**四个方向全鼓出**
    //   Hips   前0.171 后0.129 左0.199 右0.206  —— 半径 0.29，正后方鼓出 **0.161**
    // 及腰长发正好搭在 Hips 这颗球上，所以那道空气墙最明显。
    //
    // 改法：半径取前后厚度的一半、球心移到前后中点，这样球的前后两极正好落在体表：
    //   Spine1 半径 (0.173+0.200)/2 = 0.187，球心再往后 (0.200−0.173)/2 = +0.014
    //   Hips   半径 (0.171+0.129)/2 = 0.150，球心往前 (0.129−0.171)/2 = −0.021
    // 侧向会略小于体表（Spine1 差 0.03、Hips 差 0.05），这是**故意的**：头发是沿前后
    // 垂挂的，侧向本来就够不到——旧的 0.27/0.29 侧向也没贴住（体表在 0.157/0.203），
    // 换句话说旧值在任何方向上都没贴对，只是把发链整体顶远了。
    // 头球不动：这个模型的后脑没有身体网格（VRChat 惯例把发下头皮删了，射线打不到），
    // 0.3 是当初量的颅骨半径，没有更好的基准。
    springColliders: [
      { bone: "mixamorig:Head", radius: 0.3 },
      { bone: "mixamorig:Spine1", radius: 0.187, offset: [0, 0.22, 0.034] as [number, number, number] },
      { bone: "mixamorig:Hips", radius: 0.15, offset: [0, 0, -0.021] as [number, number, number] },
    ],
  },
};
...
/** 第一人称"自身近距剔除"兜底：把玩家自己网格上**距相机近于 R 的片元** discard。
 *
 *  ⚠ 这不是低头穿模的主修法。真因是**眼点位置**：PLAYER_HEAD 是 mixamorig:Head 骨，
 *  而这根骨在颈椎顶端不在眼睛处，旧的 +0.12 前移让相机卡在颅骨内部（见
...
export default function PlayerArms({ avatar }: { avatar: PlayerAvatar }) {
  ...
  useMemo(() => {
    ...
    toonify(gltf.scene, OUTLINE_W, undefined, {
      ...
      faceUnlit: true,
      // 边缘光：与铸卡师同一套（阈值来历见 toonify 里 animeShadeOf 的注释）。
      // 卡组特写是第三人称对着脸，玩家整个人贴在暗砖墙上；三阶 ramp 只分明暗档、
      // 不认视线夹角，把人从暗底里剥出来的只有这一笔。
      // ★ 不给 specBand：铸卡师那边靠 match:["hair"] 只把高光打在头发上，而玩家模型是
      //   图生 3D 出来的**单网格单材质**，没有可匹配的部件名 —— 要么全身都上（实测
      //   会把布料照成湿乳胶），要么不上。这里选不上。
      // ★ 强度 0.7，比铸卡师那边（1.0）低一档：她一身墨绿，加色白边怎么加都不过曝；
      //   玩家这套有大面积**白色泡泡袖**，同样的边缘光会把袖口顶穿。实测同机位过曝
      //   像素占比：1.0=0.251% / 0.7=0.047% / 0.55=0.004%，而均亮度三者只差 1.3/255
      //   （边缘光本来就只作用在边上）——0.7 是"轮廓还立得住、白袖不炸"的那一档。
      rimLight: { from: 0.24, to: 0.62, strength: 0.7 },
    });

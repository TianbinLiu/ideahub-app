...
//   - 只说**已经在做**的两处显式标识：合并导出成片的画面内标识（由原生合成器按
//     `data/aigcLabel.aigcBadgeSpec()` 烧进起始画面，措辞一律取 `badgeLegalClause()`）
//     与作品周边的界面标识（components/AigcBadge，2026-08-30 补上的那一半）；
//     ⚠ 别再照 `drawAigcBadge` 写"逐帧"：那个函数 2026-09-07 起**全仓零调用点**；
...
import { badgeLegalClause } from "./aigcLabel";
  ...
  terms: {
    ...
    body: (
      <>
        ...
        <Sec t="五、AI 生成内容标识">
          {/* ★ 与《AIGC 内容须知》二同源，都取 aigcLabel.badgeLegalClause()：
              这里原本写「在成片画面添加**持续显示**的标识」，2026-09-07 盖法改成
              起始画面若干秒之后就成了假话（2026-09-08 评审抓到）。 */}
          依据《人工智能生成合成内容标识办法》，本应用对 AI 生成内容做显式标识：
          {badgeLegalClause()}；作品在应用内的信息流、分区页与详情页另有「AI 生成」标识。
          任何人不得删除、篡改或隐匿这些标识。
        </Sec>
        <Sec t="六、内容授权">
        ...
        <Sec t="七、内容管理">
          违规内容可能被下架或删除，相关账号可能被限制使用。已发布作品可在编辑页修改标题、
          封面、可见性等信息；也可以「回炉重做」替换成片内容——链接与播放、点赞、评论都保留，
          作品会标注版次，收藏过它的人会收到提示，原有弹幕会被清空。被平台下架期间不能替换内容。
          对处理结果有异议，可发邮件至{" "}
          {SUPPORT_EMAIL}。
        </Sec>
        <Sec t="八、免责与变更">
    ...
    body: (
      <>
        ...
        <Sec t="五、你的权利">
          ...
          「设置 → 存储」清理本机缓存；注销账号可在「设置 → 注销账号」自助发起
          （连接服务器时）。恢复已注销的账号、要求彻底删除数据或导出数据，请联系{" "}
          {SUPPORT_EMAIL}。
        </Sec>
        <Sec t="六、未成年人">
  ...
  aigc: {
    ...
    body: (
      <>
        ...
        {/* ★★ 2026-09-07 改：这一段原文写的是角标「合成导出时逐帧写入……不可关闭，也不得以
            任何方式删除、遮挡或篡改」。
            ★★★ 2026-09-08 再改一次（评审抓到，上一版只改对了一半）：措辞不再抄字面，一律取
            `aigcLabel.badgeLegalClause()`。上一版写的「在应用内播放时画面上**持续**显示角标，
            其中合并导出的成片**逐帧**写入画面本身」两半都是假的：
              ① `drawAigcBadge` 全仓**零调用点** —— 2026-09-07 撤掉 canvas 录屏时连调用一起删了，
                 合并走原生合成器 + `aigcBadgeSpec()`，`MODE="head"` ⇒ 只烧起始画面 2.5 秒；
              ② `AigcBadge` 的渲染点是信息流 / 分区页 / 个人页网格 / 详情页 / 发布页，
                 **三个播放器里一处都没有** ⇒ "应用内播放时画面上持续显示"根本不存在。
            未经合并导出的作品（逐段生成、互动分支）文件里一个标识都没有，只靠文件名前缀。上线「保存到本地」之后不改这段措辞，等于平台一边把标识摘掉、一边告诉
            ...
            重写了 —— 编辑页那颗「🛠 回炉重做」现在真的能换掉成片内容（链接、播放、点赞、
        ...
        <Sec t="二、标识不可移除">
          {/* ★ 强调一律用 <b>，**不要**写 markdown 星号：这一段没有任何 markdown 解析，
              写了就是让用户在法务正文里读到字面的星号（2026-09-08 复核抓到，连同反引号）。 */}
          本应用依据《人工智能生成合成内容标识办法》做显式标识：一是<b>作品周边</b>——首页信息流、
          分区页、个人主页与作品详情页在作品旁显示「AI 生成」标识；二是<b>画面内标识</b>——
          {badgeLegalClause()}。
          你把作品<b>保存到本地</b>时，文件名会带 AIGC 前缀作为文件一级的标识。
          以上标识都不可关闭，也不得以任何方式删除、遮挡或篡改；对外传播时请一并保留。
        </Sec>
        <Sec t="三、素材与肖像">

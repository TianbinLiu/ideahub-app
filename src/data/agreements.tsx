...
//   - 只说**已经在做**的两处显式标识：画面右下角的角标（drawAigcBadge 逐帧写入）
    ...
    body: (
      <>
        ...
        {/* ★★ 2026-09-07 改：这一段原文写的是角标「合成导出时逐帧写入……不可关闭，也不得以
            任何方式删除、遮挡或篡改」。而实读代码的事实是：只有**剪辑页合并导出**的那一批
            把角标烧进了画面（drawAigcBadge 的调用点全仓只在 CutPage），其余作品（逐段生成、
            互动分支）的角标是 App 内的 DOM 覆盖层（components/AigcBadge），文件一离开 App
        ...
        <Sec t="二、标识不可移除">
          ...
          显示「AI 生成」角标，其中经剪辑页合并导出的成片会把该角标逐帧写入画面本身。

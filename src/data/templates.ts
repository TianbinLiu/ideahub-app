 ...
 * @throws message 可直接显示（含超时那一句；409 时是服务端的原话：上一次识别还没跑完，原样透出）
 */
export async function detectTemplateRoles(id: string, atSecs?: number[]): Promise<string> {
  ...
  try {
    ...
    //   「请求超时」会把用户推去立刻重试，而重试只会吃 409（识别中的那把并发锁：上一次还没跑完）。
    //   ⇒ 把这一档单独说清楚：等一会儿、回来看，别急着再点（铁律八：失败要带下一步）。
    // ★ 只改这一种（`TIMEOUT`）的措辞：网络不可用、401、409 那几档服务端/客户端
    //   本来就说得准，原样抛。
    if (e instanceof ApiError && e.code === "TIMEOUT") {
      throw new Error(
        t`等太久没等到回复——但这一发多半还在服务器上跑（认人最坏要几分钟），而且可能已经计费。先别急着再点（这会儿再点只会被服务器挡回来：上一次还没跑完）：等一两分钟，退出这一页再进来看看角色位是不是已经有了。`,
      );
    }
    throw e;
  }

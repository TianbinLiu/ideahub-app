...
//
// ⚠⚠ 关于 `doubao-seedream-5-0-260128`（此前一直是全站默认出图模型）：
//   2026-08-13 账单实测 —— 它在账单里叫 **Doubao-Seedream 5.0 Lite**，单价 **¥0.22/张**
//   （「文生图张数」与「图像编辑张数」同价，所以以第 1 张为参考画的第 2/3 张不用另算）。
//   这里原来写着"公开价目里查不到单价，因此不进档位表"—— **那是错的**：查不到只是因为
//   它在账单里叫另一个名字。把默认模型从它换成 4.0 的唯一理由由此不成立，
//   要不要换回来是产品决定（0.22 vs 0.20，差 10%），**但别再拿"查不到价"当理由**。
//
...
export const IMAGE_TOKENS_BY_MODEL: Record<string, number> = {
  // 0.20 元/张 ÷ 15 元/M = 13,333。账单核对 ☐ 未做（2026-08-13 打过一发真图，尚未出账）
  "doubao-seedream-4-0-250828": 13_333,
  // 0.25 元/张 ÷ 15 元/M = 16,667。账单核对 ☐ 未做（同上）
  "doubao-seedream-4-5-251128": 16_667,
  // 0.60 元/张（输出图 > 236 万像素那一档）÷ 15 元/M = 40,000。账单核对 ☐ 未做
  //
  // ⚠⚠ 这一格有**两个**没定的数，2026-08-13 打了两发对照图等账单确认（见上面那段实测）：
  //   ① 「0.02 元」到底是什么：第 2 张起的**输入图**加价（我们恒 1 张参考图 ⇒ 永不触发，
  //      本格不用改），还是每次请求的**提示词费**（⇒ 41,333），还是根本不适用、
  //      而「基础推理 1.00 元/张」才是口径（⇒ 66,667）。
  //   ② 236 万像素那条分界是否真的存在。
  //   对照实验：同一提示词、同一天各出一张 —— C=pro@1728×2304(398万)、D=pro@1296×1728(224万)。
  //   账单里 C 若约为 D 的两倍，分界成立、本格的 40,000 正确；C≈D 则分界不在这儿，
  //   顶档可以白捡一半价钱（只改 ImageTier.size 那一行）。

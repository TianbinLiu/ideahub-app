...
//   不止一个（/video-editor 独立路由页 / 提取器浮层里的两条白模路），状态形状也不同。
// ★ 引导同样由宿主声明：这里只把 trim-crop / trim-range 两个锚点落到 DOM 上；「选段与裁剪」那份讲的是白模化的钱与「AI 看哪几帧」，
//   只对缺省形态成立 —— 提取器只在 AI 白模化那条路挂（它的 aiTrimScreen），自带白模片那条路不挂。
//

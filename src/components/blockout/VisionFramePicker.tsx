...
// ★ 另有一条兜底不在这里，在服务端的提示词里：**清单之外的人也一律白模化，但一律纯白色、
//   不给颜色**（宁可让它挂不了卡，也不要出现一个列表里没有的颜色 —— 那会让作者核对时
//   看见一个清单上不存在的人偶，而没有任何人能仲裁那到底是谁）。
//   老编号方案下这条兜底的形态是"不给编号"，同一个用意。
//
  ...
  return (
    <div className="space-y-2 rounded-lg border border-slate-700 bg-panel/60 px-3 py-2.5">
      ...
      <p className="text-[10px] leading-relaxed text-slate-500">
        ...
        逐个点名替换成<b className="text-slate-300">不同颜色</b>的白模人偶（一人一色）。
        <b className="text-amber-300">看漏了人，他会变成一个纯白色的人偶</b>
        ——照样出现在画面里，但挂不了卡。
      </p>
      {/* ★★ 命中率：这一屏是**花钱之前的最后一屏**，目的是让作者多标几帧 / 减少人数。
          与提取器那一屏逐字同源（同一条实测，同样不出现化妆过的百分比）——
          两处都说是有意的：一处管"要不要做"，一处管"怎么做得更准"。 */}

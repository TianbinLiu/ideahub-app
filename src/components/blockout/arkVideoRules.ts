 ...
 *
 * ★★ 唯一实现：界面上「AI 看 N 帧」（数它的长度）、请求体、以及"有几个标记落在选段外"那句提示，
 *   读的都是这一个函数的结果。分成两处数的话，嘴上说的帧数与真正发上去的帧数会各漂各的。
 *   （这里原来写着"帧数就是钱"—— 不对：看帧那一笔是一次 chat 定额，与几帧无关，见 economy.blockoutTemplateCost。）

...
export default function ReviseBox({
  ...
  return (
    <div className="space-y-1.5 rounded-lg border border-slate-700/70 bg-black/25 px-2.5 py-2">
      <p className="text-xs font-semibold text-slate-300">✎ 返修这一段</p>
      {priceIssue ? (
        ...
        <>
          <textarea
            ...
            maxLength={120}
            disabled={disabled}
            placeholder="改哪里？例：把背景换成雨夜 / 去掉右上角的台标 / 主角衣服换成红色"
          ...
          <p className="text-[10px] leading-relaxed text-slate-500">
            拿这一段成片当参考视频重拍，只改你写的地方；时长跟随成片（{sec}s），产物<b>无声</b>（合并时可外挂音轨）；上一版会留一份可还原
          </p>
          <div className="flex gap-2">
            ...
            >
              ✎ 返修（{AI_REAL && cost !== null ? fmtTokens(cost) : "演示"}）
            </button>
            {prop.prevVideoUrl && (
              <button

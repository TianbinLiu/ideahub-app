...
export default function CastPreviewCard({
  ...
  return (
    <div className="space-y-1.5 rounded-lg border border-slate-700/70 bg-black/25 px-2.5 py-2">
      {node.castPreview ? (
        <>
          <img src={node.castPreview} alt="挂卡合成预览" className="w-full rounded-md" />
          <p className="text-[10px] leading-relaxed text-slate-500">
            合成预览：只看站位与形象对不对，不代表成片画质；改了挂法要重新合成
          </p>
        </>
      ) : (
        <p className="text-[10px] leading-relaxed text-slate-500">出片前先看一眼：把你挂的角色放进白模画面，站位不对这时候改还不花视频钱</p>
      )}
      <button
        onClick={() => void run()}
      ...
      >
        {busy ? "合成中…" : `${node.castPreview ? "重新" : ""}合成预览（${AI_REAL ? fmtTokens(ONE_IMAGE) : "演示"}）`}
      </button>
    </div>
  );
}

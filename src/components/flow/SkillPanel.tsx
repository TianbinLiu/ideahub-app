  ...
  return (
    <div className="mt-3">
      <div className="mb-1.5 flex items-center gap-2">
        <span className="mb-1.5 text-xs font-semibold text-slate-300"><Trans>我的技能（点一个 = 填进输入框）</Trans></span>
        <span className="flex-1" />
        {!saving && (
          <button
            onClick={() => {
              if (!draft.trim()) {
                setErr(t`输入条里先打好那句话，再来存成技能`);
                return;
              }
              setSaving(true);
              setErr("");
      ...
      {saving && (
        <div className="mb-2 rounded-xl border border-slate-700/70 bg-panel px-2.5 py-2">
          ...
          <input
            ...
            placeholder={t`技能名（≤${SKILL_TITLE_MAX} 字）`}
      ...
      {mine.length === 0 && !saving && (
        <p className="text-[10px] leading-relaxed text-slate-600"><Trans>还没有技能——输入条写好常用指令，点「＋ 存为技能」。</Trans></p>
      )}
      {mine.length > 0 && (
            ...
            <div key={s.id} className="flex items-center gap-2 rounded-xl border border-slate-700/70 bg-panel px-2.5 py-2">
              <button onClick={() => onPick(s.text)} className="min-w-0 flex-1 text-left">
                <div className="flex items-center gap-1.5">
                  ...
                  {s.published && (
                    <span className="flex-none rounded bg-emerald-500/20 px-1.5 py-0.5 text-[9px] text-emerald-300"><Trans>已发布</Trans></span>
                  )}
                </div>
              ...
              {marketOn && (
                ...
                >
                  {s.published ? <Trans>下架</Trans> : <Trans>发布</Trans>}
                </button>
              )}
              <button
                onClick={() => doDel(s)}
              ...
              >
                {delId === s.id ? <Trans>真删？</Trans> : <Trans>删</Trans>}
              </button>
            </div>
          ))}
        </div>
      ...
      {marketOn && (
        ...
        >
          <Trans>逛技能市场</Trans> {marketOpen ? "▴" : "▾"}
        </button>
      )}
      {marketOpen && (
        <div className="mt-1 space-y-1">
          {busy && <p className="text-[10px] text-slate-500"><Trans>拉取中…</Trans></p>}
          {!busy && sharedSkills().length === 0 && !marketErr && (
            <p className="text-[10px] text-slate-600"><Trans>广场上还没有人发布技能——发布你的第一条？</Trans></p>
          )}
          {sharedSkills().map((s) => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-2 rounded-xl border border-slate-700/70 bg-panel px-2.5 py-2">
                ...
                >
                  {installed ? <Trans>已装</Trans> : <Trans>装</Trans>}
                </button>
              </div>
            );
          })}

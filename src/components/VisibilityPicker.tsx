...
const OPTIONS: { value: Visibility; icon: "play" | "lock" | "share"; label: MessageDescriptor; hint: MessageDescriptor }[] = [
  { value: "public", icon: "play", label: msg`公开`, hint: msg`出现在首页、分区和搜索里，谁都能看` },
  ...
  { value: "unlisted", icon: "share", label: msg`凭链接可见`, hint: msg`不进首页和搜索；但拿到链接的人都能看，能被转发` },
  { value: "private", icon: "lock", label: msg`仅自己可见`, hint: msg`只有你能看到；别人拿到链接也打不开` },
];
...
export default function VisibilityPicker({
  ...
  return (
    <div className={className}>
      <div className="mb-1.5 text-sm font-semibold text-slate-300"><Trans>谁能看到</Trans></div>
        ...
        {OPTIONS.map((o) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-0.5 text-[10px] leading-relaxed text-slate-400">{t(o.hint)}</div>
            </button>
          );
        })}
      </div>

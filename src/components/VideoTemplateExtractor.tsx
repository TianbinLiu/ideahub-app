...
//
// ── 白模模板（blockout）──────────────────────────────────────────────
// 另一种输入：**任意一段视频**。AI 先看帧列出画面里有哪些人，再把他们全换成**不同颜色的**
// 白模人偶（一人一色，一次真实付费出片），产物才是模板；套用者出片走 r2v 整段复刻场景与运镜、
// 只把对应颜色的人偶换成他挂的卡。
// （2026-08-16 之前的模板是"在人偶头上印数字"，那些模板照旧按编号用 —— 方案位与两套措辞
//  见 data/templates 的 isColorMark 与 studio/blockoutPrompt 文件头。）
//
              ...
              {got.refVideo && (
                <p className="mt-1 text-[11px] text-sky-300">
                  ...
                  {/* ★ 角色位数量必须说出来：它决定套用者能挂几张卡，而"AI 只认出 2 个人"
                      ...
                      ★★ 颜色方案下**可以直接把颜色列出来**，这是一处真正的体验改善：
                      老编号方案只报个数，理由是编号不连续（实出 1/2/4/5），列出来反而误导；
                      而颜色是**按调色板顺序连续分配**的，列出来就是作者接下来要核对的那份清单。
                      ...
                      剩下的人偶是出片出坏了。 */}
                  {got.roles?.length
                    ? got.markColors?.length
                      ? ` · 识别出 ${got.roles.length} 个角色位：${got.roles.map((r) => r.label).join("、")}（套用时在编辑页按颜色逐个挂人物卡，没挂的保持人偶原样）`
          ...
          <>
            {blockoutReady && (
              <div className="mb-3 rounded-xl border border-slate-700 bg-black/25 p-3">
                ...
                >
                  <span>
                    ...
                    <span className="mt-0.5 block text-[10px] leading-relaxed text-slate-500">
                      拿任意一段视频当底：AI 把画面里的人全换成<b className="text-slate-300">不同颜色的人偶</b>
                      （一人一色），做成模板。套用者出片时整段复刻它的场景与运镜，只把对应颜色的人偶换成自己的角色
                    </span>
                  </span>
                  <span
                ...
                {/* ★★ 命中率：这一屏是**花钱之前、选文件之前**，所以这里说的目的是让作者
                    ① 决定要不要做 ② 选一段人少的。三条禁令（写在设计里，别改软）：
                      · 不许出现"AI 会准确识别""智能识别每个角色"这类话；
                      · 不许把 4/7 化妆成"成功率超过一半"—— 给的就是原始形状，带上"5 个人"这个前提，
                        脱离人数的百分比是假精度；
                      · 套用者那一侧**不提**命中率（他拿到的是作者已经核对过的模板，说了他也做不了
                        任何事，只会不敢用）。
                    ⚠ "人越少越准"这条经验来自编号方案时代（3 位历次全对、5 位从没全对过）。
                      颜色方案下**没有按人数分组的实测**，只有 5 人 4/7 与一发 4 秒 5 人 5/5 ——
                      所以只给方向性表述，**不给具体数字**。 */}
                {blockout && (
                  <p className="mt-2 rounded-lg bg-amber-500/10 px-2.5 py-2 text-[10px] leading-relaxed text-amber-200/90">
                    <b className="font-bold">先说清楚：这一步不是每次都全对。</b>
                    实测 5 个人的素材连打 7 发，其中 4 发把 5 个人全部换对了，其余每发有 1~2 个人没被换成人偶
                    —— 最常出错的是<b className="font-bold">画面正中央、看起来最像主角的那一个</b>。
                    出片之后你要对着画面逐个核对一次：对不上的位子删掉就行（不用重炼、不花钱），
                    要全对上只能重炼（<b className="font-bold">重炼要再花一次钱</b>）。
                    <b className="font-bold">画面里的人越少越准</b>，能只框一段 3~5 个人的镜头就别框一群人。
                  </p>
                )}
                {blockout && (

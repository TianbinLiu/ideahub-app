...
const WM_CORNERS = [
  {
    right: false,
    bottom: false,
    hint: msg`左上角疑似有水印或台标：白模出片会把它逐帧复刻进每一条成片，建议裁掉这块后再上传。（只是提醒，看错了直接忽略——照样可以继续。）`,
  },
  {
    right: true,
    bottom: false,
    hint: msg`右上角疑似有水印或台标：白模出片会把它逐帧复刻进每一条成片，建议裁掉这块后再上传。（只是提醒，看错了直接忽略——照样可以继续。）`,
  },
  {
    right: false,
    bottom: true,
    hint: msg`左下角疑似有水印或台标：白模出片会把它逐帧复刻进每一条成片，建议裁掉这块后再上传。（只是提醒，看错了直接忽略——照样可以继续。）`,
  },
  {
    right: true,
    bottom: true,
    hint: msg`右下角疑似有水印或台标：白模出片会把它逐帧复刻进每一条成片，建议裁掉这块后再上传。（只是提醒，看错了直接忽略——照样可以继续。）`,
  },
] as const;
              ...
              <>
                ...
                {blockout ? (
                  <p className="text-[10px] leading-relaxed text-slate-500">
                    ...
                    {busy ||
                      (blockoutBlock
                        ? t`白模那两条路现在开不了（原因见上）。上面改选「经典配方」仍然可以做模板——它不需要付费套餐，只是不做白模人偶。`

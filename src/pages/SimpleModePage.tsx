...
export default function SimpleModePage() {
  ...
  const stepNo = step === "start" ? 1 : step === "fill" ? 2 : 3;
...
function StepStart({ onPick }: { onPick: (r: "template" | "custom") => void }) {
  return (
    <div className="flex flex-1 flex-col justify-center gap-3">
      <p className="mb-1 text-center text-sm text-slate-400"><Trans>想怎么开始？</Trans></p>
      <button
        onClick={() => onPick("template")}
        className="rounded-2xl border border-brand/50 bg-brand/10 px-4 py-6 text-left"
      >
        <div className="text-base font-bold text-slate-100"><Trans>🧪 套一个模板</Trans></div>
        <div className="mt-1 text-xs leading-relaxed text-slate-400"><Trans>挑一条现成的片子，复刻它的画面与运镜</Trans></div>
      </button>
      <button
        onClick={() => onPick("custom")}
        className="rounded-2xl border border-slate-700 bg-panel px-4 py-6 text-left"
      >
        <div className="text-base font-bold text-slate-100"><Trans>✍️ 自己写一句</Trans></div>
        <div className="mt-1 text-xs leading-relaxed text-slate-400"><Trans>说一句想拍什么，AI 直接出一条短片</Trans></div>
      </button>
    </div>
  );
}
...
function StepTemplate({
  ...
  return (
    <div className="flex flex-1 flex-col">
      <p className="mb-3 text-sm text-slate-300"><Trans>挑一个模板</Trans></p>
      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto">
            ...
            <div key={row.key} className="rounded-xl border border-slate-700/70 bg-panel">
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-[10px] text-slate-500"><Trans>共 {count} 段 · 点开选一段</Trans></div>
                </div>
                ...
                <div className="space-y-1.5 border-t border-slate-700/60 p-1.5">
                  {row.parts.map((p) => (
                    <TplRow
                      ...
                      t={p}
                      currentId={currentId}
                      seg={t`第 ${(p.group?.index ?? 0) + 1}/${count} 段`}
                      onPick={() => onPick(p)}
                    />
                  ))}
                </div>
        ...
        {rows.length === 0 && (
          <EmptyState emoji="🧪" text={t`还没有可用的模板`} cta={{ label: t`← 换成自己写一句`, onClick: onBack }} />
        )}
      </div>
    </div>
  );
...
function StepCustom({ value, onChange, onNext }: { value: string; onChange: (v: string) => void; onNext: () => void }) {
  ...
  return (
    <div className="flex flex-1 flex-col">
      <p className="mb-3 text-sm text-slate-300"><Trans>想拍什么？</Trans></p>
      <textarea
        value={value}
        ...
        maxLength={400}
        autoFocus
        placeholder={t`例：雨夜霓虹的街头，穿风衣的侦探快步穿过人群`}
...
function StepGo({
  ...
  return (
    <div className="flex flex-1 flex-col">
      <p className="mb-3 text-sm text-slate-300"><Trans>确认一下就出片</Trans></p>
      {tplTitle && (
        <div className="mb-3 rounded-xl border border-slate-700/70 bg-panel px-3 py-2.5">
          <div className="text-[10px] text-slate-500"><Trans>套用的模板</Trans></div>
          <div className="truncate text-xs font-semibold text-slate-100">{tplTitle}</div>
        </div>
      )}
      <div className="mb-3">
        <div className="mb-1.5 text-[11px] text-slate-500">{tplTitle ? t`再补一句要求（可留空）` : t`要拍什么`}</div>
        <textarea
          value={plot}

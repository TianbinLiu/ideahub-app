      ...
      {planFocus && (
        ...
        >
          ...
          {node.status !== "generating" && node.error && (
            <span
              className={`min-w-0 flex-1 truncate rounded-full px-2.5 py-1.5 text-[11px] font-semibold ${
                node.status === "pending" ? "bg-amber-500/90 text-ink" : "bg-rose-500/90 text-white"
              }`}
            >
              {node.status === "pending" ? "⏳" : "✗"} {node.error}
            </span>
          )}
        </div>
      )}

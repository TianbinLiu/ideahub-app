...
| GET | `/api/live2d-models` | optional | Live2D 形象市场：`?page&limit(≤40)&sort=new\|hot&q&tag&scope=all\|installed\|mine` → `{ ok, models: Live2dModel[], total, page, limit, totalPages }`。`scope=all` 第一页最前是官方条目 `official-mascot`（`modelJsonUrl: ""`，不计 total）：App 拿到空串就用打包的 `/live2d/mascot/mascot.model3.json`；`installed/mine` 未登录 401。上传见下面的 `POST /`（**官网与 App 都走它**，App 的向导在 `/support/models/new`）。`Live2dModel` 自 2026-09-05 起多 `capabilities` / `mapping` / `license` / `takenDown` 四项，**老条目一项都没有 —— 客户端一律判否定**（`capabilities?.badges?.length`），判等值会把存量整批当成"什么都不会" |
...
| GET | `/api/personas` | optional | 人格市场列表（App 只用这一条 + install）：`?scope=all\|installed\|mine&page&limit(≤40)&sort=new\|hot&q` → `{ ok, personas: Persona[], total, page, limit, totalPages }`，Persona 含 `_id, name, description, coverEmoji, coverImageUrl, tags, styleDescriptor, price, shared, installed, purchased, isOwner, voice: VoiceSettings\|null, stats`。「给数字人装人格」= `PUT /api/companion/settings { personaId }`；`POST /:id/install` 只是收藏（书签，幂等），`DELETE /:id/install` 取消。⚠ **未登录时 `installed` / `mine` 被服务端静默降级成 `all`**（`persona.controller.js`：`if ((scope === "installed" \|\| scope === "mine") && !req.user) scope = "all"`）—— 与 `/api/live2d-models` 的「installed/mine 未登录 401」**处理方向相反**。后果要记住：token 刚过期那一拍，「我的」页签会安安静静地摆出整个市场里别人的人格，创作中心那张卡的「我的作品 N」显示的是全站总数，全程零报错。App 侧这两处（`SupportPersonasPage` 的三页签、`SupportCreatePage` 的计数）依赖路由的 `RequireAuth` 挡在前面 |
| POST | `/api/live2d-models/bundle/sign` | required | **App 大文件路径**（`ideahub-server#60`，**尚未合并 —— 线上可能仍回 404**）：Cloudinary `raw` 直传签名 → `{ ok, uploadUrl, publicId, params, chunkBytes: 6000000, maxSizeBytes: 26214400 }`。**形状与语义和 `POST /api/uploads/template-video/sign` 完全相同**（同一套分块 XHR：`params` 原样逐字段转发、每块带同一个 `X-Unique-Upload-Id` + 闭区间 `Content-Range`），只是 `uploadUrl` 落在 `/raw/upload`。传完把 `publicId` 当 `bundleRef` 交给 `/inspect` 与 `POST /`，服务器拉回来解包后删掉那份 raw。限流 5 次/分钟 + 20 次/天。⚠ **404 = 这台服务器还没有这条路由，客户端必须退回 multipart 直传 `POST /api/live2d-models`**（小包过得去；>15MB 会撞上 Cloudflare 那道 125 秒读超时，见「白模模板 · 素材上传与回收」）——App 侧 `api/uploads.uploadLive2dBundle` 回 `null` 表达这件事，向导要在进度文案里说清现在走的是哪条 |
| POST | `/api/live2d-models/inspect` | required | **创作中心（2026-09-05）** 只看不存：multipart `bundle`=zip（≤25MB）+ 可选 `entry`，**或**（大包直传后）JSON `{ bundleRef, entry? }` → `{ ok, entries: string[], entry, capabilities: { motionGroups, motionCount, expressions, hitAreas, params, paramsKnown, hasPhysics, hasPose, textures: { count, maxSide }, badges: ("motions"\|"expressions"\|"touch"\|"physics")[] }, mapping, completeness: { required: [{slot, ok}], recommended: [{slot, ok}], recommendedDone, recommendedTotal }, warnings: string[] }`；按用户 10 次/分钟。 ⚠ **`completeness[].ok` 是三态**：`true` 对上了 / `false` 确实缺 / **`null` 说不出来** —— `capabilities.paramsKnown` 为 false（包里没有 cdi3.json、读不到参数表）时 `required` 那五项**全是 `null`**。 把 null 当 false 的后果是：一个只是导出时没勾 cdi3 的正常模型被整屏判成「不能眨眼 / 不能转头」，而且永远点不动下一步（零报错）。`recommended` 那半永远是布尔。 `slot` 的取值集合（服务端 `live2dCapabilities.service.completenessOf` 一处实现）：`required` = 参数槽名 `mouthOpen` / `eyeL` / `eyeR` / `angleX` / `angleY`；`recommended` = `idle` 与 `action:<动作槽>` / `face:<表情槽>` / `touch:<触摸区>`（槽名同 companion.json）。都是英文标识符，界面上要自己配中文名（App 侧一处实现 `SupportModelNewPage.slotLabel`）。`mapping` 是自动映射（companion.json 内容）：`{ version: 1, idle, start, actions: {acknowledge…playful: 动作组\|null}, faces: {normal…cuddle: {expression?, params?}\|null}, touch: {Head…Legs: {hitAreas[], motion}\|null}, params: {mouthOpen, eyeL, eyeR, angleX, angleY, angleZ, eyeBallX, eyeBallY, bodyX, breath, cheek, mouthForm}, fit: {heightRatio, xBias} }` |
| POST | `/api/live2d-models` | required | 上传（**官网与 App 共用这一条**，App 向导在 `/support/models/new`）：multipart `bundle` + `name/description/coverImageUrl/tags/shared/personaId/voice(JSON)` + **`mapping`（JSON 字串，缺省 = 自动映射）、`entry`、`selfMade`（授权勾选）**，**或**（大包直传后）JSON 带 `bundleRef`（+ 可选 `bundleName`）代替 `bundle` → 201 `{ ok, model, warnings, entries }`；映射引用了包里没有的动作组 / 表情 / 命中区 → 400（message 含那个名字）。5 次/分钟。服务器把映射写成 model3.json 旁边的 `companion.json`，运行时 `new URL("companion.json", modelJsonUrl)` 读，404 = 用默认协议。`Live2dModel` 多出 `capabilities`、`mapping`、`license: {selfMade, agreedAt}\|null`、`takenDown`。⚠ **multipart 的文本字段到手全是字符串**（server `live2dModel.schemas.js` 自行转型）：布尔只认 `"true"/"1"/"on"/"yes"`（**false 一律别发**）、`voice`/`mapping` 要 `JSON.stringify`、`tags` 逗号分隔。这几条在 App 侧的一处实现是 `api/companion.bundleParts` |
| PUT | `/api/live2d-models/:id` | required | 作者改元数据 / 人格 / 嗓子 / **`mapping`（对象 = 校验后重写 companion.json，null = 恢复自动映射）/ `selfMade`** → `{ ok, model, warnings }` |
| POST | `/api/personas/analyze` | required | **人格向导第 2 步**：`{ materials: [{ kind: "chat"\|"posts"\|"notes", text }] (≤10 条、合计 ≤60k 字), speaker? }` → `{ ok, analysis: { catchphrases[], habits: {sentenceLength, punctuation, emoji, particles, tone}, stances[], topics[], avoids[], samples[] }, model, sampledChars }`；素材不落库；5 次/分钟 |
...
| POST / PUT | `/api/personas` `/api/personas/:id` | required | 字段上限（`persona.schemas.js`，2026-09-07 逐条登记）：`name` 1～120、`description` ≤1000、`coverEmoji` ≤8、`coverImageUrl` ≤2000、`tags` 数组或逗号串（草稿那侧 ≤12 条 × ≤30 字）、`price` 0～100000 整数；`style` = `summary`(≤2000) / `catchphrases`(≤50 条 × ≤120) / `stats`(≤30) / `stanceHint`(≤500)，外加 `tone`(≤300) / `addressUser`(≤60) / `greeting`(≤300) / `examples`(≤12 组，各 ≤300) / `boundaries`(≤12 条 × ≤120)。⚠ **客户端别把这几个数再往小里收**：其中大半是 AI 生成之后填进表单的，收小了屏幕上就红着一句「超了 N 字」，而它其实发得出去（假警报比没警报更坏）——App 侧一处实现在 `companion/personaWizard.PERSONA_LIMITS`。**`PUT` 的 `style` 是 PATCH 语义**（2026-09-18 起）：只改请求里真的带了的键，没带的保持库里的值；显式发 `""` / `[]` 照常算清空；整个不带 `style` = 一个字不动。原来是「zod 给没带的键补空值 → 整份替换」，官网编辑器只认 summary / catchphrases / stats / stanceHint 四个键，于是 App 向导做出来的人格拿到官网上改一次价格，tone / addressUser / greeting / examples / boundaries 就被清空（200、零提示）。⚠ 以后 style 再加字段，**老客户端不必回传它不认识的键**；也别依赖「少发一个键 = 清掉它」。`POST` 不变（没带的键补缺省值）。`remixable` 与 `license` **服务端还没有**（治理 P5），z.object 默认 strip，发了是静默丢掉，界面上也不许说「已留痕」；回包 `style` 同样带这些字段，`styleDescriptor` 拼进语气 / 称呼 / 边界；Persona 多 `takenDown`。列表 `GET /api/personas` 现在在数据库里搜索 / 排序 / 分页，`q` 大小写不敏感匹配 name / description / tags |

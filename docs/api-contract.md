...
| 角色位 | 无（整段只有一个主体） | 有（`roles[]`，套用者逐个人偶挂卡；标记有编号/颜色两种方案，见下） |
...
### 角色位标记有两种方案（2026-08-16 起并存，**判据是 `markColors` 的存在性**）

| | 编号方案（存量，2026-08-16 之前建的） | 颜色方案（新建的） |
|---|---|---|
| 人偶长什么样 | 白色，头上印阿拉伯数字 | **通体一色**，一个角色位一种颜色 |
| `roles[].label` | `"1"`/`"2"`/`"4"`（稳定但不连续） | 色名（`"绿色"`…，按调色板顺序连续分配） |
| `markColors` | **没有这个键** | 有（白模化那一刻真正发出去的那份清单） |
| 套用提示词 | `把带编号的白色人偶替换为对应角色：编号1=张三。`＋**必须**有一句「把编号全部去掉」 | `把有颜色的人偶替换为对应角色：绿色=凛。`，**没有**擦除句 |
| 没挂卡的人偶在成片里 | 白色（看起来像风格化） | **带着自己那个颜色**（这是一处真实的质量回退，产品侧如实告知并引导挂满） |

换方案的理由全是实测：编号 5 个角色位从来没有一发 5/5 全对（最好 4/5，还带重号，实出过
2/2/1/1/5）；"头部四面各印同一个数字"**从没被执行过**（每发只印一面，哪一面还不可控）；
编号会被逐帧原样复刻进成片（实拍：换上去的角色后脑顶着「1」）。根因是这个模型把数字当
"贴在当前这一帧上的二维贴纸"、**不维持跨帧对象恒等性**，而"任意角度读到同一个号"恰恰要求它。
颜色是**材质**，任何角度都对，两个老毛病一次消失，而且实测颜色**不会渗进角色卡**
（挂在绿色位上的凛，长袍还是黑金的）—— 所以颜色**不需要**擦除句。

⚠ **但这不是"修好了"**：颜色方案同素材同参数 7 发只有 4 发全对（≈57%），失败形状高度一致
（画面正中央那个"最像主角"的没被抹掉、相邻两色互换）。作者核对 / 删位 / 换色那一整套机制
是**兜底**，必须保留。

⚠ **发布顺序：先发 App，再切服务端。** App 侧的颜色分支对老服务端完全无害（老服务端不发
`markColors` → 全部走编号分支 → 与之前逐字相同的行为）。反过来先切服务端，还没更新的用户
建出来的是颜色模板、而他们的 App 不认这一位 → 走编号分支 → 输入框里出现
`把带编号的白色人偶替换为对应角色：编号绿色=凛`。那句话**一眼就是坏的**（这正是选择
"`label` 直接装标记 token"而不是"label 留数字 + 另加 color 字段"换来的好处：后者会写出
一句看起来完全正常的 `编号1=凛`，用户不会起疑，钱花完才拿到一段换错人的片子），而且出现在
**花钱之前**的可编辑输入框里，更新 App 即恢复。可接受，但没必要制造。
...
{ _id, id(字符串化的 _id，两个都回), ownerId(身份判定唯一依据), authorName(显示快照，会过时),
  ...
  markColors?: [{ label, swatch? }],          // ★ 标记方案位：**存在 = 颜色方案，缺失 = 编号方案**（派生，不落库 swatch）
...
- **`roles`（白模 V2）**：`label` = 白模人偶身上那个**可寻址的标记**，`desc` = 这个标记在
  ...
  0. **最多 9 条**（`BLOCKOUT_MAX_ROLES`，2026-08-15 起）：白模化时最多分配到 9 个，核对端点
     ...
     画面里超出这个数的人**照样白模化但不给标记**（颜色方案下是纯白色；同下面「清单之外的人」
     那条兜底）。
     ...
     ⚠ 颜色方案下 9 是**没验过的一档**：白模化提示词按 9 人算约 709 字，而实测 594 字通过、
     605 字就开始顶穿预算（抹外观那几句先垮）。6 人以上已经在出过问题的区间里，产品侧用
     文案劝到 5 人以内。改这个数是一次跨仓决策。
  1. **判存在性**（`roles?.length`），不判等值 —— V1 老模板整个键缺失，回空数组会让客户端
     分不清"老模板"和"新模板但一个人都没认出来"（后者根本建不出来，见下面 blockoutize 的
     「roles 为空整句拒」）。
  2. **`label` 是字符串，原样用**。两种形态并存，靠 `markColors` 分辨（见下一条）：
     颜色方案是色名（`"绿色"`），编号方案（存量）是阿拉伯数字且**稳定但不连续**
     （2026-08-15 F5 实测：一发四人实出 1/2/4/5）。别按下标推、别拿 `roles.length` 当最大编号、
     点名时**原样用 label**（不重编、不换近义色名 —— "绿色"写成"青色"就是换错人）。
  3. **`labelConfirmed` = 作者对着成片核对过了**。为假时那份 label 只是服务端按视觉清单
     顺序**分配的猜测** —— 与画面对不上时，套用者给「绿色」（老模板是"3 号位"）挂上张三，
     模型会老老实实换掉画面上真正的那一个（另一个人），**钱照扣、零报错**。所以未核对的
     模板**不许发布**（见下 `PATCH /publish` 的第二道门）。客户端读不到这一位时按**未核对**
     处理（往多提醒一次那侧退）。
- **`markColors`（标记方案位，2026-08-16 起）**：白模化**那一刻真正写进提示词**的那份颜色
  清单，逐字、按序（如 `["绿色","黑色","蓝色","紫色","红色"]`）。响应里派生成
  `[{ label, swatch? }]`（`swatch` 由服务端按色名现查调色板得到，**不落库**，只给 App 画色块）。
  1. **存在即颜色方案，缺失即编号方案。** 判据只写存在性 —— 线上那两个还在用的老模板
     （人偶身上印数字、身体是白的）天然缺这一位，于是套用时走编号版提示词，一个字不受影响。
     反过来写成 `!== "number"` 会把存量整批翻面：画面上根本没有绿色人偶、提示词却说
     「把绿色人偶替换为…」——**当场作废且零报错**。这是这次改动的头号红线。
  2. **不可变**：`PATCH /roles` 只写 `doc.roles`，永远不碰它（客户端塞进来也被 zod strip）。
     让作者改得动方案位 = 让他把一个颜色模板标成编号模板，套用侧当场整份错。
  3. **必须与 `roles` 同一批写入**（阶段一 `BlockoutJob`、阶段二 `BranchTemplate`）：白模化
     提示词在阶段一就发出去了、凭据 TTL 24 小时，发版正好夹在两阶段之间时，只有"凭据里记着
     当初发的是哪一套"才能保证 finish 出来的模板与那段视频真正的样子一致。在途的老凭据没有
     这一位 → finish 出编号方案模板 → **正确**。
  4. **真有才出这个键**，绝不 `|| []` 兜底（同 `realDurationSec`）：空数组与"老模板"在下游会被
     压成同一个值，而两者的处置相反。
  5. **调色板只有服务端一处**（`blockoutize.service.MARK_PALETTE`）。App 仓**不许**出现任何
     色名或色值常量：颜色的文字来自 `roles[].label`，色块来自这里的 `swatch`。于是"两边相等"
     从靠约定变成结构上不可能不等。查不到 swatch 时服务端不出这个键，App 画中性灰块 + 纯文字，
     **永远不猜一个颜色**。
...
- **`source` 服务端存、但不出任何响应**：`{ publicId, startSec, durSec, crop:{x,y,w,h} }`，
  ...
  正当用途（同 `cloudinaryPublicId`）。客户端提交 `roles`/`source`/`markColors` 一律被 zod
  strip —— 收 `roles` = 让提交方自己写"1 号位是谁"，收 `source` = 让用户自己标价（`durSec`
  就是 r2v 的计价输入时长），收 `markColors` = 让提交方自己宣称这个模板是哪种方案（套用侧
  当场整份错且零报错）。**唯一的例外**是作者的角色位确认（`PATCH /:id/roles`，见下），
  而它也**只收 `roles`**。
...
| POST | `/api/branch/templates/blockoutize` | required（**3 次/10 分钟**） | **V2 白模化 · 阶段一**。body `{ publicId, startSec, durSec, crop:{x,y,w,h}, frameTimes?, title, intro, coverUrl, videoTier?, aspect?, note? }` —— 提交的是**四组数不是 URL**（变换地址由服务端自己拼）。`frameTimes` 见下「看哪几帧」，**自动模式不带这个字段**。成功 `201 { ok, jobId, taskId, durSec, frames, roles[], markColors?, expiresAt }`（`markColors` 见上：存在即颜色方案，**阶段一就要给** —— 模板还没建出来时核对入口就要知道该按颜色说话还是按编号说话）（**钱在这一刻花掉**；`frames` = 服务端**真正看了几帧**，App 报价与它不等时以它为准并如实说一句）。失败一律带 `billed`（见下） |
...
| GET | `/api/branch/templates/blockoutize/pending` | required | **掉线恢复**：本账号还没取回结果的凭据 `{ ok, jobs: [{ jobId, taskId, durSec, title, roles[], markColors?, expiresAt, createdAt }] }`。App 进模板市场时拉一次，摆出取回入口 |
...
| PATCH | `/api/branch/templates/:id/roles` | required（仅作者，**仅 pending**） | **作者核对角色位**（白模 V2）。body `{ roles: [{ label, desc }] }`，1~9 条（`BLOCKOUT_MAX_ROLES`），**整份替换**（"少给一条 = 删掉那个角色位"，见下），落库时 `labelConfirmed=true`。这是**唯一收客户端 roles 的端点**；`markColors` **不收也不动**（改得动方案位 = 套用侧当场整份错）。见下 |
...
7. **一次 r2v edit**（F4 的"点名"：提示词把每个人的外观特征逐个写进去 ——
   泛指"所有人物"只换配角、主角不动，两发对照实测）。标记是**一人一色、通体一色**
   （2026-08-16 起；此前是在头上印数字，换掉的理由见上面「两种方案」那一节）：
   颜色是材质，转身/侧对/被挡住再露出来都还是那个颜色，而标记正是套用者
   "点这个人偶挂这张卡"的唯一连接键 —— 看不见就没法开始挂卡。
   **最多 9 个**（见上面 `roles` 的铁则 0），清单之外的人照样白模化但一律**纯白色**、不给颜色；
   提示词里还有一句「人偶身上不要出现任何数字或文字」。
   **⑥ 与 ⑦ 之间就是阶段一的终点**：
   任务被受理即落凭据并 201 返回（`jobId`/`taskId`/`durSec`/`roles`/`markColors`/`expiresAt`）；
   ⚠ `markColors` 必须**在这一步落进凭据**（与 `roles` 同一批）：提示词是在这里发出去的，
   而凭据 TTL 24 小时 —— 发版夹在两阶段之间时，只有凭据里那一份能保证 finish 出来的模板
   与那段视频真正的样子一致。
8. 【阶段二】向方舟核实 → 产物**转存 Cloudinary**（F12）→ 用 **② 号窗口**复核产物本身
   （它要当下一发的输入）；
9. 建模板 `status=pending`、`roles`（全部 `labelConfirmed:false`）、`markColors`（从凭据原样搬，
   不重报）、`source` = 那四组数，并把凭据置为已取回（幂等的锚点）。
   在途的老凭据没有 `markColors` → 建出编号方案模板 → **正确**（它的视频上印的确实是数字）。
...
- **兜底（提示词，两种模式都要）**：画面里出现了清单之外的人，**也一律白模化，但不给标记**
  （颜色方案下是**纯白色**；老编号方案下是"不给编号"）。宁可让它挂不了卡，也不要出现一个
  清单里没有的标记 —— 作者核对时会看见一个清单上不存在的人偶，而**没有任何人能仲裁**那到底
  是谁；套用者看着它却挂不上，只会以为坏了。
  ⚠ 这也是**纯白绝不进调色板**的原因：白已经被"清单之外的人"占用了。调色板里出现白，
  等于"路人"和"某个角色位"在画面上长得一样。同理排除灰（夹在黑白之间三方混淆）与
  银/金（那是**材质**不是颜色，与"哑光塑料"直接打架，而且没法核对）。
...
**为什么这条端点非有不可**：白模化落库那一刻的 `label` 是服务端按视觉清单顺序**分配的猜测**，
而成片上人偶身上真正的标记是方舟画上去的，两者常常错位 —— 编号方案下"稳定但不连续"
（F5 实测一发四人实出 1/2/4/5）；颜色方案下命中率 ≈57%（7 发 4 发全对），最常见的失败是
**画面正中央那个最像主角的人根本没被换成人偶**，其次是相邻两色互换。错位时套用者给「绿色」
（老模板是"3 号位"）挂上张三 —— 模型老老实实换掉画面上真正的那一个（另一个人），
...
- `label` 收**任意非空字符串**（≤8 字符）：**不校"是数字"、不校连续、不锁个数**，
  也不校"是不是调色板里的色名"。1/2/4/5 是编号方案实测的正常输出，校连续等于把正确的确认
  判成非法；色名两个汉字，8 字符的上限两种形态都放得下（所以这次换方案 `maxlength`
  一处都不用改）。作者还可以补上视觉漏认的人、删掉它多认的一条（服务端**整份替换**，
  不逐条 merge）。
  ⚠ **`markColors` 不在 body 里、也不许被这条端点改写**：它是"这个模板是哪种方案"的判据，
  由白模化那一刻的服务端说了算。作者一按「核对无误」就把方案位擦掉 = 套用侧当场整份错
  且零报错 —— 这是这次改动里最容易漏的一处（zod 的 strip 是第二道，handler 里逐字段重建
  `{label, desc}` 时**只写 `doc.roles`** 是第一道）。
- **`label` 不许重复** → 400 整句（重了的话套用侧的 `label → 卡` 映射会**静默互相覆盖**，
  用户看到的是"我给两个人各挂了一张卡，结果只换了一个"）。这也正是**颜色 token 直接装进
  `label`**（而不是"label 留数字 + 另加一个 color 字段"）换来的：重号闸自动就是颜色重号闸，
  一行代码都不用加；连接键全仓也仍然只有一个。整句提示按方案分（"编号「1」出现了两次" /
  "颜色「绿色」出现了两次…如果画面上真的有两个绿色人偶，请把其中一个位子删掉"）。
...
- 编号方案：实测同一段 5 人素材出过 `2/2/1/1/5`（两组重号，3 和 4 整个没出现）与 `3/1/1/4/5`。
  ...
  改三个位子的号 + **删掉另外两个**。
- 颜色方案：命中率 ≈57%（同素材同参数 7 发，4 发 5/5 全对）。观察到的两种失败是
  **有个人根本没被换成人偶**（3 次失败里 3 次都是画面正中央那个最像主角的）与**相邻两色互换**；
  颜色重号在这 7 发里一次都没出现过（但闸还在，文案也还留着）。前者只能删位，后者对调两行。
没有"删"这条路，作者打开模板发现对不上时唯一的出路是**再花一次钱重炼整段**。
⚠ 所以这套机制在颜色方案下**必须原样保留**并跟着改文案：命中率 57% 意味着它是兜底，
不是"已经修好了所以可以砍掉"。
...
- `confirmTemplateRoles()` —— 角色位核对，**成功后把本机 `roles` 一起改写**：出片时点名用的
  ...
  ⚠ 它**只发 `roles`**，`markColors` 一个字都不带（那是方案位，见上）。
- `isColorMark()` / `markSchemeOf()` / `MARK_NOUN` / `swatchOf()`（`src/data/templates.ts`）——
  「这个模板是哪种标记方案 / 界面上怎么称呼它 / 某个色名画什么色块」的**全 app 唯一实现**。
  提示词（`studio/blockoutPrompt`）、核对面板、挂卡面板、`flowStore.applyCast` 的错误句
  全部问它们。**App 仓不许出现任何色名或色值常量** —— 颜色的文字来自 `roles[].label`，
  色块来自 `markColors[].swatch`（服务端派生），调色板只有服务端一处。
- 方案位要穿过 `rolesOf`/`markColorsOf` → `apiToTemplate` → `flowStore.applyTemplate` 快照 →
  `FlowPage.castEditorState` → `VideoEditorPage.parseState` 五跳，其中后三跳都是**逐字段重建**
  （已知会静默丢字段）。所以每一跳都显式带这一位；漏一跳的后果是颜色模板被当成编号模板 ——
  好在那时写出来的是 `编号绿色=凛`，**一眼就是坏的**且在花钱之前（这正是选 label 装 token
  的理由）。⚠ `parseState` 里这一位**必须可选**：缺它绝不 `return null`，否则一个从模板页
  正常点进来的用户会撞上"这一页需要从上传或模板页进来"。
...
| **存量 V2 模板（没有 `markColors`）** | `isColorMark` 判否 → **编号方案** → 套用走老提示词（含那句「把编号全部去掉」）→ **一个字都不受影响**。线上那两个好模板（`都市主角群舞转场`、`宗主垫脚舞`）走的就是这条。这是本次改动的头号约束 |
| 在途 `BlockoutJob`（24h 凭据，发版夹在两阶段之间） | 凭据里没有 `markColors` → finish 出编号方案模板 → **正确**（它的视频上印的确实是数字） |
| 颜色方案服务端 × 老 App（不认 `markColors`） | 走编号分支 → 输入框里出现 `编号绿色=凛`，**一眼就是坏的**，且在花钱之前的可编辑输入框里；更新 App 即恢复。所以发布顺序是**先 App 后服务端**（见上「两种方案」那一节） |
| 本机模板库 `templates.v1` 里建模板时写下的老记录 | 缺 `markColors` → 判编号 → 靠 `refreshRemoteTemplate` 回写自愈（**mine 与 shared 两条路都要回写**，否则作者自己那台设备上会一直把自己的颜色模板当编号模板） |

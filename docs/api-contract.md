...
| POST | `/api/live2d-models/inspect` | required | **创作中心（2026-09-05）** 只看不存：multipart `bundle`=zip（≤25MB）+ 可选 `entry` → `{ ok, entries: string[], entry, capabilities: { motionGroups, motionCount, expressions, hitAreas, params, paramsKnown, hasPhysics, hasPose, textures: { count, maxSide }, badges: ("motions"\|"expressions"\|"touch"\|"physics")[] }, mapping, completeness: { required: [{slot, ok}], recommended: [{slot, ok}], recommendedDone, recommendedTotal }, warnings: string[] }`；按用户 10 次/分钟。`mapping` 是自动映射（companion.json 内容）：`{ version: 1, idle, start, actions: {acknowledge…playful: 动作组\|null}, faces: {normal…cuddle: {expression?, params?}\|null}, touch: {Head…Legs: {hitAreas[], motion}\|null}, params: {mouthOpen, eyeL, eyeR, angleX, angleY, angleZ, eyeBallX, eyeBallY, bodyX, breath, cheek, mouthForm}, fit: {heightRatio, xBias} }` |
| POST | `/api/live2d-models` | required | 上传（此前只在官网做，App 向导起也走这条）：multipart `bundle` + `name/description/coverImageUrl/tags/shared/personaId/voice(JSON)` + **`mapping`（JSON 字串，缺省 = 自动映射）、`entry`、`selfMade`（授权勾选）** → 201 `{ ok, model, warnings, entries }`；映射引用了包里没有的动作组 / 表情 / 命中区 → 400（message 含那个名字）。服务器把映射写成 model3.json 旁边的 `companion.json`，运行时 `new URL("companion.json", modelJsonUrl)` 读，404 = 用默认协议。`Live2dModel` 多出 `capabilities`、`mapping`、`license: {selfMade, agreedAt}\|null`、`takenDown` |

...
- **不计费**（不动 token 钱包）：合并不调用任何 AI，端上原本也是免费的。成本落在 Cloudinary
  ...
  限流 6 次/分、以及**每人滚动 24 小时 900 秒的产出预算**（`COMPOSE_DAILY_SEC`，超了回
  ...
  预算有两条容易写错的地方（都被回归测试钉住了）：
  ① **重试也计费**（`spentSec` 累计，不是按登记行数）——只按行算的话，一份稳定失败的配方
  可以无限重试，闸门等于不存在（实测过：6 次真编码、预算只记 1 份）；
  ② **先预约再核账**，不是先查后写——查完再写中间隔着几个 await，并发受理各读各的旧值，
  实测 6 发并行能放进 1799.99 秒（上限 900）。
- BGM 至少 5 秒（`minBgmSec`）：`e_loop` 是实测行为不是文档契约，我们只验到 `e_loop:3`；
  一段 0.1 秒的"音乐"铺满 300 秒会算出 `e_loop:2999`，那是谁都没验过的形状。太短整句拒，
  不悄悄替用户改循环次数。
- 变换串里 `,` 与 `:` 是**参数分隔符**，所以写进变换的 public_id 一律先过字符集
  （`assertSafePublicId`）。段落那条路的归属判据是整体形状匹配、天然干净；**BGM 那条**走的是
  `ownedCloudinaryAsset`（本来只为回收封面而写、不关心字符集），构造一条
  `…/ideahub/x,fl_splice,l_video:<别人的段落>/<自己的>.mp3` 就能把任意参数塞进一条我们签名
  并付费的变换 —— 拦在拼装那一处，将来多一条图层路径也自动受保护。
...
- 产物是 `ideahub/branch-videos/<userId>-<ts>-<配方指纹8位>-merged`（带 tag `ideahub-merged`），
  与段落同目录同归属形状 —— 它本身也能再被当成一段拼进下一次合并（实测过）。
  ★ 名字里必须带指纹：只有 `<userId>-<ts>` 的话，同一个人**并发**的两发（不同配方 = 不同任务，
  指纹去重管不着）会在同一毫秒算出同一个 public_id，后一发把前一发的成片静默换掉，
  而两条记录都还是 done。发布链路原样沿用（非方舟域的 http(s) 会被保留，不再多搬一次）。
- **轮询会自愈**：进程被 kill（优雅退出不等后台任务）时登记行会停在 pending，
  客户端只会一直转圈直到 48h TTL 把行删掉、变成「没有这个合并任务」。所以 GET 端点看到
  超时未动的 pending 会当场判失败并说明可以重试；「是不是僵尸」的判据只有一处
  （阈值 = 单趟超时 × 2，从超时推导而不是写死，否则调大超时就会把活任务判死）。

...
| POST | `/api/personas/generate` | required | **第 4 步**（扩参，向后兼容）：`{ chatText?(≥20 字), hint?, basics?: {name, role, relation, intro, addressUser}, questionnaire?: {extroversion, rationality, formality, humor, talkative, politeness, emotional, catchphrase, addressUser, language, emoji, taboos…}, analysis?, only?: string[], draft? }`（chatText / analysis / basics 至少一个；`only` 必须带 `draft`）→ `{ ok, draft: { name, description, coverEmoji, tags, style: { summary, catchphrases, stats, stanceHint, tone, addressUser, greeting, examples: [{user, reply}], boundaries } }, model }`；5 次/分钟 |
| POST | `/api/personas/preview-chat` | required | **第 5 步 试聊**：`{ draft: { name, description?, coverEmoji?, tags?, style? }, messages: [{role, content}] (≤20，最后一条须 user), lang? }` → SSE，事件与 `/api/companion/chat` 相同（`sentence` / `token` / `done` / `error`）；草稿不落库；20 次/分钟 |
| POST / PUT | `/api/personas` `/api/personas/:id` | required | `style` 多收 `tone`(≤300) / `addressUser`(≤60) / `greeting`(≤300) / `examples`(≤12 组，各 ≤300) / `boundaries`(≤12 条)；回包 `style` 同样带这些字段，`styleDescriptor` 拼进语气 / 称呼 / 边界；Persona 多 `takenDown`。列表 `GET /api/personas` 现在在数据库里搜索 / 排序 / 分页，`q` 大小写不敏感匹配 name / description / tags |

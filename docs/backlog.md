...
- **`data/promptSchemes.ts` 必须保持叶子**（只依赖 types 与零依赖的 `data/schemeSlotIds`——后者一个 import 都没有，成不了环）。联网那半边在
...
- **「自己传图做卡片」接上方案体系**：人物卡的图位不再写死三格，由所选方案定
  （与提取那条路同一套方案库；此页不出图，方案只定结构）。人物卡图位按**方案图位 id** 键存
  （`schemeShots`；2026-09-11 起从按 tag 改成按 id，名字只是显示名，见 `data/schemeSlotIds`），
  与非人物卡的 kind 库互不相通 —— 涉及人物卡的换卡种**什么都不丢**。内置方案里同一格共用同一个 id
  （fullBody / faceCloseup），换方案时 id 对得上的留、对不上的收起来**并说明**（与 changeType 同一条纪律）。
  ⚠ 按 id 认与按名字认**有收有放**（没有照片被删；清单见 `data/schemeSlotIds` 文件头）。收：两套不相干的自建方案里同名的格子
  不再共用照片（两边都是老方案 / 市场装来的、且 role 相同时除外），名字相同 role 不同的也不共用；自建方案里**存过的**格子
  后来改名成内置原名（全身立绘 / 面部特写），或删掉再加一格内置原名的，也不再与内置方案共用（「这一次改了名就认回内置」那一版
  评估过：分两次保存改过去再改回来，那格自己的照片会从此够不着，所以不采用）；删掉一格再加一格同名的，旧照片在这一套里够不着。
  放：就地改名照片跟着格子走；另存内置方案的副本改了名仍与内置方案共用；正文与某个内置图位完全相同的老格子 / 市场格子认回内置 id。
  改 role 就换成 role 变体、改回原 role 回到原 id（三类 id 都记原 id，与中间改没改名无关，复核第 5 轮）。重装自己的方案（「已装 · 用它」）、
  删掉之后从市场装回来时，先认本机记下的「服务端那一版」（`promptSchemes.noteServerSlots`：推上去 / 装回来时写进 localStorage，
  App 重启也在），逐格内容相同就按位置接回原 id（记下的那一格本机已经删了、而之后新加的一格对得上这个位置时，让给新格子）；
  对不上再对齐本机那份（换过 role 的格子按原 role 认回原 id）；删掉之后装回来时，「本机那份」是同一次会话里删掉那一刻的那一版（只在内存里，
  `promptSchemes.removedSlots`）—— 否则发布后本机改过 role / 名字加正文、隔一次重启再装，或者删掉一格再加回同名的、传了照片再装，
  那几格的照片从此够不着。仍然够不着的（复核第 6 轮实测）：记录与服务端那一行对不上（别的设备又推过、回包没收到）时名字和正文都改过的格子；
  App 更新之前就改过 role、或名字加正文一起改过的格子。这几种情况下那句
  「「X」里没有「Y」这一格，你传的那张先收起来了」点名的格子，目标方案里看得见一格同名的空格子（从改了名的那套换回内置方案时，
  点名的就是内置方案标题里写着的那一格）—— 措辞留给接 Lingui 改内置图位名的那一批一起改（目录冻结的这一批不动文案）。

...
- **P2-a 简约 refMode 预算沿用了 Seedream 的理由**：refMode 的图直接进 Seedance，
  却按「Seedream 画一张帧塞不下多主体」的启发式砍到 3 张。白模路 30 张多主体实测成立
  （G0：3 卡各就各位）。可放宽为 multiChar 式分配（每人物卡 face+body、其余各 1，
  上限按档位协议 9/30），需一发实测钉住再合。
- **P2-b 方案台帧画真人卡会被动漫化**：`framePrompts` 的厚涂词对真人照片参考同样生效。
  现有产品答案是"真人出片走 MiniMax 档或 asset:// refMode"，首尾帧承接路本来就不是
  真人主路——记录在案即可，修 P1-a 时顺手让「有真人卡」也压掉画风词更好。
...
（情感互动/整活/变身/节日/带货）；④ 模板卡片做减法（名称+循环预览+生成键，参数进二屏）；
⑤ 方案升级为可分享 Skill（agent 输入条 "/" 唤起）；⑥ 点名句标准化 @槽位语法
（与即梦万能参考/Runway image_N 同构）；⑦ 方案台加运镜 chips（受控词表，≤3 叠加）；
⑧ 模板声明输入槽位并前置校验（缺卡禁用+写明原因，本仓早有同款纪律）；
⑨ 初始模板全自产（上面红线的正面版）；⑩ 每周一个热点特效的运营节奏。
...
**留一条没动的（要花真钱 A/B 才敢动）**：截断刀口方向。现状是"保尾巴（绑定句/点名句）、
砍正文尾"；而各家证据都说**头部权重最高、尾部最先被忽略**——理论上该把身份句/点名句
挪到提示词**开头**、砍只砍尾部风格词。但白模 V2 的点名句形态是真机实测钉下来的
（G0/A2 那批），挪位置前先拿一发付费段 A/B 验过再合，别按调研直接改。

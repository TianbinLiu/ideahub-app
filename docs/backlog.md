...
### 2.11.2 三条真缺陷（都已逐行核实，暂不修）

**① 承接段上「前半段圈选」重画出来的帧被丢弃，而钱照收**（有钱，最该先修）
- `segmentGen.ts` 圈选循环逐条跑 `refineFrame`（一次真的 Seedream 图生图），
  `a.atSec < half` 的写进 `first`；**紧接着**下一行 `if (input.carryFrame) first = input.carryFrame;`
  把它整张覆盖。而报价 `flowStore.nodeCost` 按 `annRedrawCost(node.anns.length)` 收**全额**。
- 后果：凡承接段（第 2 段起且上一段已出片），圈在前半段的每一处都是"图钱花了、图作废"，
  只有文字要求随 `reqs` 进提示词。全程零报错，进度行还写着「按圈选改画面 1/N…」。
- 修法：把"哪几条圈选真的会重画"收成**一个**导出函数，报价与真跑读同一个（方向是**少收钱**，
  所以报价≠实扣的风险方向安全）；并出声说明「本段承接上一段结尾，开头画面不重画，
  你圈的这 N 处只作为文字要求发出去」。
  ⚠ **别用"让编辑后的帧赢、丢掉承接帧"来修** —— 那会直接打断接缝，而承接是这条产线优先级最高的事。
- **触发条件**：任何人反馈"圈了没用"；或下次动 `annRedrawCost` / 圈选那条路时顺手修。

**② 自定义参考视频那条支路整条漏发音色样本**（最贵的一档）
- `segmentGen` 的素材参考支路调 `composeSegments` 时给了 `refImages` / `refVideoUrl` / `refVideoSec`，
  **没给 `refAudios`**；而 `voiced` / `voiceOk` / `voiceLine` 要到后面才算，这条支路早就 return 了。
- 这条路只在 ultra 上开（`refVid` 只有它 true），而 ultra 的 `tier.audio === true`、
  协议上本来就是 reference 模式 —— **完全够格发**。
- 后果：挂了带 🔊 的人物卡、写了「」台词，拿回随机音色；连「为什么没带上」那几句说明也一并跳过。
  与 CLAUDE.md 里 `settleSegment` 那条「第二条支路抄漏两行」是同一形状。
- 修法：`voicedCardsOf` / `voiceOk` / `voiceLine` 提到公共段，这条支路一起带上，
  "带不了"的 notes 走同一处判断。⚠ 音频不占 `<图片N>` 编号，改的时候别把两套编号串了。
- **触发条件**：下次动这条支路，或有人反馈"自定义参考视频出来的台词不是我卡里那个声音"。

**③ 音色点名句被提示词硬顶从尾巴静默切掉**
- `voiceLine` 是拼在硬顶**之外**的（`plot` 已经按 `room` 截到 `VIDEO_PROMPT_MAX`，
  然后 `${plot}${voiceLine}`），最终 `real.ts` 的 `slice(0, VIDEO_PROMPT_MAX)` 从尾巴下刀。
  ⇒ 正文写满时它**必然**整句消失。
- 注释写明这是**有意的取舍**（「它丢了只是音色随机（软降级），正文被截才是内容错」）——
  取舍成立，**问题是它是静默的**：`cut` 那句警告只按正文与 room 算，一个字不数它，
  而参考音频照样发出去（零加价）⇒ 两张以上带声音的卡时音色随机。
- 修法（不推翻原取舍）：检测到 `voiceLine` 被切掉时，把这件事加进 `cut`/notes 那句话里。
- **触发条件**：下次动提示词预算，或有人反馈"音色对不上"。

### 2.11.3 两条零成本改进（暂不做）

**④ 承接段的点名句该说出"图片1 是上一镜的最后一帧"**
- `customRefPrompt` 现在恒说「图片N是这段视频的第一帧画面，视频从它开始」——
  **没说它其实是上一镜的尾帧、要接着往下演**。而"从这一帧开始"与"接着这一帧演"对模型是两件事，
  原帖那位作者整段经验的核心就在这个区别上。
- 对照：剧情那一步反而说过（`real.ts` 喂给豆包的「本段开头画面已经确定（上一段的收尾画面），
  剧情必须从那一瞬间直接继续」），**出片提示词里一个字没有**。
- ⚠ 前置：`chain` 这个标志**根本没传进 `segmentGen`**（只活在 flowStore 里），
  所以出片那一刻我们自己都不知道该不该说这句话。要先把它传下去。
- 代价：吃 `VIDEO_PROMPT_MAX` 里约 25 字（room 已把 tail 算进去，不会多一处计算）。
  收益不确定 ⇒ 做的话要照 `design/ab-bind-syntax.mjs` 的形式拿一发对照实测，别凭直觉合。
- **触发条件**：与 ① 同一次改（都在 segmentGen 承接那一段）。

**⑤ 「段与段无缝」这句话该随档位改口**
- 承接的硬度随档位变（见 2.11.1），而界面上**四处**无条件写着"无缝"：
  `FlowCanvas`（承接说明、插中间帧那句）、`FrameCard`（「视频从开头帧无缝续拍」）、
  `SegSettings`（「从上一段的真实结尾画面接着拍」）。
  唯一说过实话的是换档确认卡 `TierRow`「起止画面不再是硬约束」，但它只在**换档那一刻**出现。
  （另：`FlowCanvas` 在"多图+参考视频"那一支下方有一句常驻实话，闸不同，覆盖不到承接这条。）
- ⚠ 改的话判据必须是 `segmentGen` 导出的**同一个**表达式（`framesAsRefs` 自己也读它），
  否则就是本仓最熟的那种分叉；`TierRow` 那条 `!cur.refImg && next.refImg` 也要改读同一处。
- ⚠ 往吓人方向说错不比往放心方向说错高尚：软引导 ≠ 接不上，措辞写「不是硬保证」而不是「接不上」。
- **触发条件**：与 ④ 同一次改。

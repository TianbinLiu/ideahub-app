...
| 背景卡 | ~~1~2 张~~ **V3（2026-09-06）：不发图**，只以「故事背景：…」一句文字参与 | 不发 | `materialText` 末尾单独一句 |
| 道具卡 | 1~2 张，绑定句=「实物参考」；同卡多图并进一句（防"两件罗盘"） | 同上 | 同上 |
| 风格卡 | 1~2 张，绑定句=「只沿用画风/材质/色调光影，别画内容」——**但见 P1-a**；V3 起出片句（idLine）含镜头语言 | 同上 | `frameArtStyle` 与 `materialText` 都读 idLine |
...
5. ✅ **上架前那三件“比支付更挡路”的已于 2026-09-03 全部落地**（都是产品功能，不是提交时补材料能过的）：
   - **拉黑用户**：server `/api/blocks` + app `BlockButton` + 「已拉黑的人」页。
     ★ 没复用 `/api/messages/blacklist` —— 那条前面串着三道**私信申请域**的防滥用闸（「对方回复过你才能拉黑」），
     而「我回过他一句、他开始骚扰我」恰恰是最该能拉黑的场景 —— 一个可能 403 的拉黑等于没有拉黑。两套写同一张表。
   - **儿童安全标准页（CSAE）**：`https://ideahubs.org/child-safety`（ideahub-client）。
     举报理由同步加了 `csae` 并**顶到待处理队列最前**（server 的 `Report.URGENT_REASONS` + `priority`）——
     那一页写着「这一类举报优先于其他所有举报」，得有东西兑现它。
   - **审核员测试账号**：server 的 `scripts/seedReviewer.js`（幂等，在生产服务器上跑）。
     刻意不给 admin（免扣费 + 能看举报队列）、不预先同意用户协议（协议前置正是要给审核员看的）。
   ⚠ 剩下的都是**流程项**：封闭测试 12 人× 14 天、身份验证 + 设备验证、安全测试文档。
   ⚠ `docs/play-store-checklist.md` 的合规那一节 2026-09-03 **整节重写**：旧版写着
   「纯前端、无账号、无网络数据收集」「Data safety 全选不收集」—— 照那个填是**申报不实**（下架 + 封号）。
...
- **保存到本地：一键进相册（MediaStore 原生插件）**。2026-09-07 上线的 v1 是「先落 App 私有 Cache，
  再由用户在系统分享面板里选去处」—— 安卓不让 App 直接写相册，而我们**一行 Java/Kotlin 都没写**。
  要真进相册得自写原生插件：`minSdk 24` ⇒ API 24–28（`WRITE_EXTERNAL_STORAGE` + 运行时申请）与
  29+（`MediaStore` + `IS_PENDING`）两套代码路径不可回避，而本仓现在**没有任何运行时权限申请代码**可参照。
  ⚠ `DownloadManager` 不是捷径：它与 `data/jobs`「任务本身随进程死」的内存票语义正面冲突。
  做之前先量一次：v1 上线后有多少人点了「分享 / 另存为」却没选到相册（现在这个数一个都没有）。
- **非作者下载的产品口径**。v1 收敛到「只能存自己发布的作品」（判据一处：`videoDownload.planDownload`
  里的 `isMyAuthor`），是因为**三份协议里没有任何一条为"下载别人的成片"兜底**：《用户协议》四只约束
  "你上传的素材要有权利"，全文没有授予其他用户转存他人成片的许可；而「凭链接可见」那一档作者选它时
  接受的代价只是"链接可转发"，有了下载之后代价升级成"文件永久留在陌生人相册里，收不回"，
  这句话**从不对作者显示**。⇒ 要放开必须先拍板，且至少配一个作者侧的「允许别人保存」开关
  （那是服务端字段，跨仓五处同步），不是改一行判据的事。
- **下载文件的元数据隐式标识**。v1 的 AIGC 标识只有文件名前缀 `AIGC-`（`AIGC_MARK_POLICY = "filename-only"`，
  主人 2026-09-07 拍板），而文件名是可以被任意重命名的。《人工智能生成合成内容标识办法》要的隐式标识
  是写进**文件元数据**（mp4 的 udta/moov、webm 的 Tags）。做它要么在服务端转存那一步写进去（一次性、
  对存量作品要回填），要么在本机改容器（等于把整条成片拉回 JS 堆，与 v1「原生下载、不过 JS 堆」的前提冲突）。
  ⇒ 归属应当是**服务端转存那一步**，不是下载链路。

## 「保存到本地」转成 MP4（2026-09-07 评审时撤下，等拍板）

**想解决的问题（实测，不是猜的）**：线上相当一部分成片是剪辑页 MediaRecorder 导出的
**vp9 webm**，安卓相册/播放器对它支持很差 —— 用户存完打不开，只会以为"下载坏了"。
2026-09-07 对线上真实成片实测：同一条片 2,308,023 字节的 vp9 webm 经 Cloudinary `f_mp4`
变成 730,055 字节的 avc1 mp4，`Content-Disposition` 也正确（`fl_attachment:<名字>` 里的名字
**只能 ASCII**，中文哪怕 URL 编码过也一律 400、回一张 image/gif 错误图）。

**为什么先撤下来**（评审当天的三条代价，都还没被拍板买单）：
1. 它是**规格 §9.4 明令禁止**的那件事（「不生成任何 Cloudinary 派生地址：`fl_attachment`、
   `l_text`、`q_auto`、`so_/du_` 一概不用」），而这条扩张没有经过拍板；
2. **配额没预算**：切一次开关，`probeSizes` 就对每一段打一发 HEAD 到派生地址，
   一发就当场触发一次计费的 Cloudinary 变换。9 段的分支作品 = 一次点击 9 次转码，
   而用户还没决定要不要下载；
3. **爆炸半径**：谁去后台开了 strict transformations，就同时打死整条下载链路 ——
   而"服务端零改动、零新依赖"正是规格 §3.1 特意写下的约束。
   另外首次请求的派生产物常常没有 `Content-Length`，落盘校验会退成 unverified。

**要做的话，先定这三件**：① 转码配额的月度预算与告警；② 探测与下载**合成一发**
（把 HEAD 探到的 total 传进 `downloadOne`，别为同一个文件触发两次转码）；
③ strict transformations 打开时的降级路径（退回原地址 + 一句人话，不是静默失败）。

⛔ 无论怎么做，派生地址**一个字都不许回流**进 `segments[].videoUrl`：服务端有两处
"只认不带变换的地址"的实现（`videoCompose.branchVideoName`、
`templateVideoAsset.ownedRecyclableAsset`），回流会同时打死服务端合并与删作品时的资产回收，
且**两件都零报错**。

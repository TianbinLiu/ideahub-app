...
**Phase B（下一批）：节点「自定义」——纯帧版**。拍板过的设计映射（三条硬约束定型）：
- 首尾帧模式**不能**同时带参考图/视频（方舟同步 400，钉在 arkClient）⇒ 「多图参考」
  只能先走**融图**合成一张，再当首帧/尾帧 —— 融图原语已存在（FuseFrameSheet）。
- 方舟**没有中间帧参数**（单段只有 first/last_frame）⇒ 用户传 N 张关键帧 = 自动铺
  **N-1 段**，相邻段共享边界帧 + 既有"真实尾帧承接"= 剪辑无缝。与逐段流水线同构。
- 工坊 / 工作流 / 简约三处入口；跳过方案台（plan 三拍）直接"帧 + 提示词 → 炼"；
  计费走现有帧模式价（`nodeCost`，不开新价目）。
- **示例视频不在本批**：任意本地视频当 `reference_video` 会被 server 的 r2v 闸整句拒
  （只认登记过的模板视频，防白嫖视频输入计费）。要支持得加"用户素材视频"合法来源 +

...
- §四 7 已落第一条：**结构化技能**（`studio/structuredSkills`）—— 技能从"一句话"升级为 `{ steps[], parse(schema), confirmAt[], cost }`，第一条官方技能「剧本 → 分镜字段」：整篇剧本（≤2000 字）→ 模型按 `ShotSpec` 拆成 ≤8 段 → 形状检查（不合的段整段丢）→ 确认卡 → 铺成流水线（每段一套已挑定方案，走 useApplyTemplate 守卫）。入口在画布「/」面板的「官方技能」。没做：技能市场里的结构化技能（服务端 schema 四处一起加）、用户自建结构化技能。

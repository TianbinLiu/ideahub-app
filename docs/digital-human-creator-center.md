...
> ⚠ **2026-09-18 起「人物模型」部分被 [`character-system-v2.md`](character-system-v2.md) 取代**：产品负责人拍板不再用 Live2D，改为 **Spine + 静态立绘并存**（可切换），市场从「Live2D 模型」改为「角色模型」（立绘 / Spine / 两者），并加上角色卡导入与三级记忆。本文里人格、声音、治理的部分仍然有效；Live2D 上传向导、companion.json 映射层等在新设计的 P4 退场。

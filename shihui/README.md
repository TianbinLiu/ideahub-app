...
**全库已炼成并过审（2026-08-13）**：20 首 / 80 段 / 100 关键帧，生成 ≈¥190 + 返工 ≈¥24。
质检流程：10 个视觉审片 agent 逐帧对照诗句与 scene 初审 + 失败帧独立二次复核——
100 帧中 10 帧维持不合格（零误报），全部定位到 **scene 措辞给了模型发挥空间**：

| 事故 | 教训（已回写进对应 scene 注释） |
|---|---|
| 《夜宿山寺》「危楼」画成现代摩天楼群，链式 i2i 传遍全部 5 帧 | 古建筑必须写死「飞檐木构古寺楼阁」；**种子帧带错整首跟着错**，首帧质量值得人工盯 |
| 《江雪》「万径人踪灭」雪径踩满脚印 | 否定意象（无人/无鸟/无痕）要写成显式否定 + 正面描述（"平整如新雪"） |
| 《独坐敬亭山》末句帧仍有群鸟大云 | 同上；跨句的叙事递进（鸟尽→云去→只剩山）模型不会自己记住 |
| 《小池》尖尖角画成全展开荷叶 | 物体的**生长阶段/形态**要写死（"未展开的卷筒状"），不能只写名词 |
| 《咏鹅》鹅掌脱体悬浮、《草》首帧枯黄 | 部位要声明与主体相连；颜色状态（青绿茂密）要当主语写 |

manifest 以磁盘重扫为准；返工 = 删坏帧连带受影响的段（`t_i` 坏则 `seg_i`/`seg_{i+1}` 同删）再续跑。

...
## ⚠ 结论先行：这套工具对 Tsumire 不适用，已实跑验证后放弃

管线本身跑通了（下面有完整数据），但**没有把结果上线**，三条实测理由：

**① 衣服底下没有身体可推。** `body.002` 不是完整皮肤——顶点密度全压在两端：
脚（Z 0.012~0.049）4906 个、头颈（Z 0.677~0.714）5595 个，中间整个躯干四肢每
5% 带只有 34~643 个。VRChat 惯例把衣下皮肤删光了。所以"把衣服推出身体"在躯干
根本无事可做。

**② 真实穿模是姿势产生的，静态外推解决不了。** 摆到运行时的垂手站姿再量：
...
绑定姿势下的外推只覆盖姿势态穿模的 **16%**，对 Overall 完全无效。姿势态穿模的
正解是碰撞体/物理（参见 breastPhysics 的 medialKeep），不是静态外推。

**③ 推内层衣物会顶穿外层。** 把 socks 对着身体推 2mm，它直接顶穿了外面的鞋——
渲图上鞋口撕裂、鞋面破出白斑。**只能推最外层**。推 Shoes 本身也不划算：131 个顶点
最大位移 18mm 在小鞋上压出可见折面，而最深穿模只从 −15.1mm 改善到 −14.4mm。

**什么时候该用它**：模型自带完整封闭的身体皮肤、且穿模在绑定姿势下就存在、且目标
衣物是最外层。三条同时成立才值得跑。
...
**不能跨 GLB 按顶点下标比位移。** Blender 的 glTF 导出器会重排顶点——实测把原始
GLB 原样进出一次，`shirt` 就有 2473/2688 个顶点"位移"、最大 336mm（而 socks/Shoes
恰好 0）。要用与顺序无关的指标（穿模计数、包围盒），别用逐顶点位移。
`shirt` 顶点数还会从 2688 变成 2694（接缝处裂点）。

**光看数字会漏掉回退。** socks 那次量测显示穿模 71→2 是"改善"，渲图才暴露它顶穿了
鞋。改完一定要渲图对照。

...
//   打进我们的 APK 属于搬运（CLAUDE.md 版权那几条）。这里自己生成一张同题材的
//   「人像 + 扫脸识别框」示意图，达意即可。
// ★ 示例主体固定用一个虚构角色（银发星星发饰的少女——与工坊看板娘同世界观但不是她本人），
//   四张图共用同一主体，用户一眼能对比"同一个人四种方案产出什么"。
 ...
 */
const CLEAN_BODY = "的全身立绘：纯白色背景，无任何背景元素与文字；全身完整可见，站姿自然";
...
const JOBS = [
  ...
  {
    file: "realface",
    // 自绘"扫脸认证"示意：真人像 + 识别框。写实向但**虚构人物**，不搬火山素材
    prompt:
      `一位真实感的亚洲年轻女性正面免冠肖像，纯浅色背景，神情自然。面部叠加半透明的青色人脸识别扫描框：` +
      `四角对焦括号、横向扫描线与稀疏的面部特征点，科技感生物识别界面风格。画面下方留白。无文字无水印。`,
  },
];
...
for (const d of todo) {
  try {
    const url = await seedream(d.prompt);

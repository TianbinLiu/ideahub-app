...
//
// 用法（在仓库根目录）：node design/gen-create-covers.mjs .
...
/** 统一构图：竖版，人物在中上部，下方留暗区压字 */
const LAYOUT =
  "竖版构图 9:16，少女位于画面中上部并占据视觉中心，全身或半身入镜；" +
  "画面下方三分之一是干净的暗色区域（几乎没有细节），方便叠加文字；" +
...
const JOBS = [
  ...
  },
  {
    ...
    prompt:
      ...
      `深色哑光金属桌面，一副耳机搁在桌上。冷青与深蓝为主调，边缘有精准的发光刻度线。${STYLE}${LAYOUT}`,
  },
  {
    file: "simple.jpg",
    prompt:
      ...
      "场景：极简的浅粉与奶白渐变空间，身旁飘着一颗小星星和一小片云，脚边有一个简单的圆形播放按钮图标，" +
      `大量留白，没有多余杂物。柔和的粉紫色调，柔光。${STYLE}${LAYOUT}`,
  },
];
...
console.log("① 出角色定妆照…");
const sheetUrl = await seedream(CHAR_SHEET, "1440x2560");
const sheetBuf = await download(sheetUrl);
writeFileSync(resolve(ROOT, "design/character-ref.jpg"), sheetBuf);
